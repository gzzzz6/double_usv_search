...
**11. 需要直接复制给 DeepSeek 时，用 text 代码块整段投喂**

如果用户要把指令整段复制到 DeepSeek，尤其是包含很多命令、路径、验收标准时，最终版本优先放进 `text` 代码块中。这样界面会显示为 `text`，用户可以一键复制整段清单，同时不会把 `bash`、`powershell` 等语言标签混入命令。
...
- 最终投喂版只使用 `text` 代码块，不使用 `bash`、`powershell`、`python` 等语言标签。
- 代码块外可以有一句简短说明，例如“下面这份可以直接投喂 DeepSeek”。
- 代码块内不要夹杂解释性寒暄，只保留任务清单、命令、验收标准和汇报格式。
- 命令尽量一行一条；如果必须多行，明确写明“以下多行属于同一条命令”。
- JSON、配置、路径可以直接贴原文，但要明确“这是文件内容”或“这是命令”。
- 对 DeepSeek 的完成汇报要求保持固定格式：修改文件、运行命令、结果路径、校验结果、异常事项。

普通说明文档可以保留 Markdown；真正交给 DeepSeek 执行的最终版本应使用 `text` 代码块整段投喂，减少复制噪声和执行歧义。
