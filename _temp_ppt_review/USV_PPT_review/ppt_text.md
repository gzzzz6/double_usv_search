...
## Slide 07: 多源搜索状态建模
chars=332, text_blocks=19, pictures=1, tables=0, charts=0, groups=0
多源搜索状态建模
➢
...
## Slide 09: 单艇信息驱动路径规划
chars=182, text_blocks=7, pictures=2, tables=0, charts=0, groups=0
单艇信息驱动路径规划
➢
单艇路径规划框架
9
...
单艇自主规划决策状态转移闭环流程
滚动时域控制思想
由于多源信息场（强度场/Staleness/GP线索场）随着USV的实时探测与环境演化而不断动态刷新，不适合进行一次性长期静态规划。
...
## Slide 12: 12
chars=245, text_blocks=15, pictures=1, tables=0, charts=0, groups=0
12
集中式编队共享信念状态
双艇集中式协同搜索决策
➢
...
chars=192, text_blocks=19, pictures=2, tables=0, charts=0, groups=0
双艇集中式协同搜索决策
➢
责任区偏置与路径冲突安全
13
...
## Slide 15: 实验设计与结果分析
chars=447, text_blocks=33, pictures=2, tables=0, charts=0, groups=0
实验设计与结果分析
➢
实验设置与评价指标
15
...
## Slide 17: 实验设计与结果分析
chars=471, text_blocks=8, pictures=3, tables=1, charts=0, groups=0
实验设计与结果分析
➢
双艇协同搜索结果
17
...
重复搜索显著降低：重复观测比例 (Δduplicate) 降低了 4%~8%（见右图），整体效率和发现全部目标的成功率得到明显改善。

## Slide 18: 实验设计与结果分析
chars=364, text_blocks=15, pictures=2, tables=1, charts=0, groups=0
实验设计与结果分析
➢
anomaly-aware 消融实验
18
：UCB 与 anomaly-aware 对比
...
## Slide 19: 总结与展望
chars=380, text_blocks=24, pictures=2, tables=0, charts=0, groups=0
总结与展望
➢
项目总结
20
...
3
双艇协同
在共享状态下进行 coordinated 顺序分配，用 residual_map 抑制重复覆盖。
4
实验验证
...
## Slide 20: 总结与展望
chars=326, text_blocks=19, pictures=2, tables=0, charts=0, groups=0
总结与展望
➢
本项目的不足
23
...
团队状态
双艇采用 centralized shared state；没有建模通信延迟、丢包或分布式一致性。
规模验证
当前只实现双艇协同基线；三艇及以上的调度复杂度与收益没有作为主线验证。
实验平台
实验在二维栅格仿真中完成，尚未接入真实 USV 船体、传感器误差和海况约束。
后续研究展望
真实环境
加入动态障碍、海况扰动和传感器误差，检验已知静态地图假设放宽后的表现。
更多艇协同
在保持可解释性的前提下扩展到多艇规模，研究任务分配、冲突处理和计算效率。
仿真验证
接入更接近实船控制的仿真或硬件在环平台，验证路径执行与感知更新闭环。

## Slide 21: 23
chars=28, text_blocks=4, pictures=2, tables=0, charts=0, groups=3
23
2025年5月
自强不息，独树一帜
感谢您的倾听！
