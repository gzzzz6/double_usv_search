"""Phase 5D-9: live planner callback continuous HoloOcean search.
...
P5D3 = _load_module(SOURCE_5D3_PROBE, "phase5d9_p5d3_detection_source")
...
def _phase_config(pre_git_status: List[str]) -> Dict[str, Any]:
    return {
        ...
        "target_placement_source": "phase5d9_fixed_openwater_cell_on_baseline_planner_forward_fan_path",
...
def _scenario_config(
    *,
    ...
    return {
        "name": "phase5d9_{0}".format(run_kind),
    ...
    *,
    ...
    def _act_and_capture(
        ...
        for usv_id in (0, 1):
            ...
            if stabilization_tick or recenter_tick:
                left = 0.0
                right = 0.0
                dist = 0.0
...
def _run_live_case(
    *,
    ...
    adapter = ReusableMainlinePerceptionAdapter(adapter_id="phase5d9_{0}".format(run_kind))
    adapter.reset_episode("phase5d9_{0}".format(run_kind))
    launch_ok = False
    error = ""
...
def _run_compile_log(path: str) -> None:
    ...
    result = subprocess.run(
        [
            ...
            "-m",
            "py_compile",
            os.path.join(PHASE_DIR, "scripts", "live_planner_standoff_capture_search_probe.py"),
            os.path.join(PHASE_DIR, "scripts", "live_planner_standoff_capture_search_audit.py"),
        ],
        cwd=os.path.abspath("."),
        check=False,
        text=True,
...
def _build_summary_md(summary: Dict[str, Any]) -> str:
    lines = [
        "# Phase 5D-9 Live Planner Callback Continuous Search Summary",
        "",
        ...
        "",
        (
            "Phase 5D-9 closes the static SphereAgent core migration path: baseline_GP chooses each step, HoloOcean executes and captures live sensors, the production adapter emits the mainline detected_mask, and the original update_found_mask/all_found chain terminates the search."
            if summary.get("phase_goal_completed") is True
            else "Phase 5D-9 exercised the real planner callback and live HoloOcean capture path, but did not close the all_found chain: live detection produced no accepted shared SphereAgent candidate under the planner-driven trajectory, so the original found_mask/all_found termination was not triggered."
        ),
        "",
    ]
    return "\n".join(lines)
...
def run_live_planner_callback_search() -> Dict[str, Any]:
    ...
    print("Running 5D-9 live calibration/reference planner callback run")
    ...
    print("Running 5D-9 live target planner callback run")
    ...
    print("Running 5D-9 live teammate-only planner callback run")
