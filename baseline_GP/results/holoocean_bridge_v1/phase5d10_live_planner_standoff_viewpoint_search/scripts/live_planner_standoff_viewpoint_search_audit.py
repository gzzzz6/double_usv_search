"""Audit for Phase 5D-10 live planner callback continuous search."""
...
PROBE_SCRIPT = os.path.join(PHASE_DIR, "scripts", "live_planner_standoff_viewpoint_search_probe.py")
AUDIT_SCRIPT = os.path.join(PHASE_DIR, "scripts", "live_planner_standoff_viewpoint_search_audit.py")
...
AUDIT_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d10_audit.json")
AUDIT_MD = os.path.join(PHASE_DIR, "reports", "phase5d10_audit_summary.md")
...
def _build_md(checks: Dict[str, bool], metrics: Dict[str, Any]) -> str:
    lines = [
        "# Phase 5D-10 Live Planner Callback Continuous Search Audit Summary",
        "",
        ...
        "- Standoff Viewpoint Hold Enabled: `{0}`".format(metrics.get("standoff_viewpoint_hold_enabled")),
        "- Target Standoff Hold Tick Count: `{0}`".format(
            metrics.get("target_standoff_viewpoint_hold_tick_count")
        ),
        "- Target Accepted Candidate From Standoff Hold: `{0}`".format(
            metrics.get("target_accepted_candidate_from_standoff_viewpoint_hold")
        ),
...
def main() -> None:
    ...
    checks["phase_completion_fields_consistent"] = (
        ...
        and (
            (
                ...
                and summary.get("phase_status") == "live_planner_callback_standoff_viewpoint_all_found_closed"
            )
            or (
    ...
    checks["standoff_viewpoint_hold_enabled"] = (
        config.get("standoff_viewpoint_hold_enabled") is True
        and summary.get("standoff_viewpoint_hold_enabled") is True
        and int(config.get("standoff_viewpoint_hold_ticks", 0) or 0) > 0
        and int(summary.get("target_standoff_viewpoint_hold_tick_count", 0) or 0) > 0
        and int(summary.get("teammate_standoff_viewpoint_hold_tick_count", 0) or 0) > 0
        and str(config.get("standoff_viewpoint_anchor_source"))
        == "baseline_GP_planner_committed_segment_lookahead_not_target_truth"
        and any(row.get("standoff_viewpoint_hold_tick") is True for row in tick_trace)
        and any(row.get("standoff_viewpoint_hold_tick") is True for row in events)
        and any(row.get("standoff_anchor_cell") is not None for row in tick_trace)
        and any(row.get("standoff_anchor_cell") is not None for row in events)
        and all(
            row.get("standoff_anchor_source") == "baseline_GP_planner_committed_segment_lookahead"
            for row in events
        )
    )
    ...
    metrics = {
        ...
        "standoff_viewpoint_hold_enabled": summary.get("standoff_viewpoint_hold_enabled"),
        "target_standoff_viewpoint_hold_tick_count": summary.get(
            "target_standoff_viewpoint_hold_tick_count"
        ),
        "target_accepted_candidate_from_standoff_viewpoint_hold": summary.get(
            "target_accepted_candidate_from_standoff_viewpoint_hold"
        ),
