"""Phase 5C-4G far-distance boundary probe for RGB signature + fan RangeFinder.

This phase answers a narrow question: with the target centered in front of the
camera, does the current RGB signature plus fan RangeFinder method still
produce a found event at farther distances?

Distances tested: 35 m, 40 m, 45 m, 50 m.
"""
...
CONFIG_JSON = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "rgb_multiray_front_far_distance_config.json"))
SCENE_RESULTS_JSON = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "rgb_multiray_front_far_distance_scene_results.json"))
DETECTION_EVENTS_JSON = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "rgb_multiray_front_far_distance_detection_events.json"))
TICK_TRACE_JSON = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "rgb_multiray_front_far_distance_tick_trace.json"))
TICK_TRACE_CSV = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "rgb_multiray_front_far_distance_tick_trace.csv"))
MATRIX_JSON = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "rgb_multiray_front_far_distance_matrix.json"))
MATRIX_CSV = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "rgb_multiray_front_far_distance_matrix.csv"))
SUMMARY_JSON = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "rgb_multiray_front_far_distance_summary.json"))
GIT_STATUS_JSON = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "rgb_multiray_front_far_distance_git_status.json"))
SUMMARY_MD = os.path.normpath(os.path.join(PHASE_DIR, "reports", "rgb_multiray_front_far_distance_summary.md"))
OVERVIEW_PNG = os.path.normpath(os.path.join(PHASE_DIR, "reports", "rgb_multiray_front_far_distance_overview.png"))
...
DISTANCES_M = [35.0, 40.0, 45.0, 50.0]
...
def _phase_config(pre_git_status: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    scenes.append(
        {
            ...
            "target": {
                ...
                "location": _offset_world(sv0_world, RGB_SIGNATURE_CALIBRATION_DISTANCE_M, 0.0, 0.0),
                "rotation": [0.0, 0.0, 0.0],
                "forward_m": float(RGB_SIGNATURE_CALIBRATION_DISTANCE_M),
                "lateral_m": 0.0,
                "yaw_deg": 0.0,
            },
        ...
        scenes.append(
            {
                ...
                "role": "front_far_distance_boundary",
                "include_target": True,
                "target": {
                    "agent_type": TARGET_AGENT_TYPE,
    ...
    return {
        ...
        "purpose": "far-distance boundary probe for current RGB signature plus fan RangeFinder adapter",
        "scope": "front-center static target distances 35/40/45/50m",
...
def _scenario_config(scene: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "name": "phase5c4g_{0}".format(scene["name"]),
        "world": "OpenWater",
        "package_name": "Ocean",
        "main_agent": "sv0",
        "agents": agents,
...
def _build_summary_md(summary: Dict[str, Any]) -> str:
    lines = [
        "# Phase 5C-4G Far-Distance Boundary Summary",
        "",
...
def run_capture() -> Dict[str, Any]:
    ...
    print("Phase 5C-4G capture finished: effective={0}, failed={1}".format(summary["effective_distances_m"], summary["failed_distances_m"]))
    return summary
