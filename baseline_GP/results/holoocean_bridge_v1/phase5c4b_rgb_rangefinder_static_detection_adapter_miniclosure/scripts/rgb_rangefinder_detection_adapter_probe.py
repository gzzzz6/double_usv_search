...
HoloOcean RGBCamera + RangeFinderSensor evidence creates one audited target
detection event, and that event drives the runtime found/hit/GP/search_info
post-update chain.
...
def _phase_config(pre_git_status: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "purpose": "RGB+RangeFinder sensor-evidence-driven found event mini-closure",
        "scope": "controlled frontal static target only",
        "not_claimed": [
        ...
        "controlled_simplifications": [
            "target actor is placed one 10m grid cell directly in front of sv0",
        ...
        "rgb_signature_detector": {
            ...
            "signature_calibration_source": "target_calibration_scene",
        },
        "scenes": [
            {"name": "baseline", "include_target": False, "role": "background_reference"},
            {"name": "target_calibration", "include_target": True, "role": "target_signature_calibration"},
            {"name": "detection_step", "include_target": True, "role": "sensor_found_event_source"},
        ],
        "map_npz": MAP_NPZ,
...
def _scenario_config(scene: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "name": "phase5c4b_{0}".format(scene["name"]),
        "world": "OpenWater",
        "package_name": "Ocean",
        "main_agent": "sv0",
        "agents": agents,
...
def _make_detection_events(scene_results: Dict[str, Dict[str, Any]], config: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    calibration_rgb = scene_results.get("target_calibration", {}).get("first_rgb_raw")
    detection_rgb = scene_results.get("detection_step", {}).get("first_rgb_raw")

    calibration_sig = _rgb_diff_signature(calibration_rgb, baseline_rgb)
    detection_sig = _rgb_diff_signature(detection_rgb, baseline_rgb)
    baseline_sig = _rgb_diff_signature(baseline_rgb, baseline_rgb)

    target_signature_colors = sorted(set(str(v) for v in calibration_sig.get("signature_colors", [])))
    detection_colors = set(str(v) for v in detection_sig.get("signature_colors", []))
    target_overlap = sorted(list(set(target_signature_colors) & detection_colors))
    rgb_has_target_signature = len(target_overlap) >= RGB_TARGET_OVERLAP_MIN

    baseline_range = scene_results.get("baseline", {}).get("rangefinder_summary", {})
    detection_range = scene_results.get("detection_step", {}).get("rangefinder_summary", {})
    rangefinder_hit = bool(detection_range.get("hit", False))
    sensor_detected_target = bool(rgb_has_target_signature and rangefinder_hit)

    no_target_false_positive = bool(
        len(set(target_signature_colors) & set(str(v) for v in baseline_sig.get("signature_colors", []))) > 0
        or bool(baseline_range.get("hit", False))
    )
    event = {
        "event_name": "detection_step_sensor_adapter_event",
        "scene": "detection_step",
        ...
        "sensor_evidence": {
            "RGBCamera": {
                ...
                "calibration_scene": "target_calibration",
                "baseline_scene": "baseline",
                "target_signature_colors": target_signature_colors,
                "scene_signature_colors": sorted(list(detection_colors)),
                "target_signature_overlap": target_overlap,
                "changed_pixels": detection_sig.get("changed_pixels"),
                "change_bbox_xyxy": detection_sig.get("change_bbox_xyxy"),
                "identity_supported": bool(rgb_has_target_signature),
            },
            "RangeFinderSensor": {
                "role": "range/object-presence evidence only, not target identity",
                "present": bool(detection_range.get("present", False)),
                "hit": bool(rangefinder_hit),
                "min_positive_range_m": detection_range.get("min_positive_range_m"),
                "raw": detection_range.get("raw"),
                "identity_supported": False,
            },
        },
        "rgb_has_target_signature": bool(rgb_has_target_signature),
        "rangefinder_hit": bool(rangefinder_hit),
        "rangefinder_min_positive_range_m": detection_range.get("min_positive_range_m"),
        "found_trigger_rule": "rgb_has_target_signature and rangefinder_hit",
        ...
        "actor_truth_used_for_detection": False,
        "target_truth_used_for_detection": False,
        "rgb_artifact": scene_results.get("detection_step", {}).get("artifacts", {}).get("rgb_preview_path"),
    }
    negative_event = {
        "event_name": "baseline_no_target_negative_check",
        "scene": "baseline",
        "expected_target_present_for_audit": False,
        "perception_sensors": ["RGBCamera", "RangeFinderSensor"],
        "rgb_has_target_signature": False,
        "rangefinder_hit": bool(baseline_range.get("hit", False)),
        "sensor_detected_target": False,
        "no_target_false_positive": bool(no_target_false_positive),
        "truth_used_for_detection": False,
        "actor_truth_used_for_detection": False,
        "target_truth_used_for_detection": False,
    }
    conclusion = {
        ...
        "detection_scene_signature_colors": sorted(list(detection_colors)),
        "rgb_target_signature_overlap": target_overlap,
        "rgb_has_target_signature": bool(rgb_has_target_signature),
        "rangefinder_baseline_hit": bool(baseline_range.get("hit", False)),
        "rangefinder_detection_hit": bool(rangefinder_hit),
        "rangefinder_min_positive_range_m": detection_range.get("min_positive_range_m"),
        "sensor_detected_target": bool(sensor_detected_target),
        "no_target_false_positive": bool(no_target_false_positive),
        "truth_used_for_detection": False,
        "composite_rule_satisfied": bool(sensor_detected_target),
    ...
    return [negative_event, event], conclusion
...
def run_capture() -> Dict[str, Any]:
    ...
    print(
        "Phase 5C-4B capture finished: sensor_detected_target={0}, rgb={1}, range={2}".format(
            capture_conclusion["sensor_detected_target"],
            capture_conclusion["rgb_has_target_signature"],
            capture_conclusion["rangefinder_detection_hit"],
        )
    )
...
def _runtime_update_from_sensor_event(
    ...
    trace = {
        ...
        "physical_execution_reason": "controlled frontal sensor capture only; no search-policy movement in 5C-4B",
...
def _build_summary_md(summary: Dict[str, Any]) -> str:
    return """# Phase 5C-4B RGB+RangeFinder Static Detection Adapter Mini-Closure

## Status
- Probe Completed: `{probe_completed}`
- Terminated Reason: `{terminated_reason}`
- Sensor Detected Target: `{sensor_detected_target}`
- RGB Has Target Signature: `{rgb_has_target_signature}`
- RangeFinder Hit: `{rangefinder_hit}`
- Found Trigger Rule: `{found_trigger_rule}`
- Truth Used For Detection: `{truth_used_for_detection}`
- Found Mask Final: `{found_mask_final}`
- Find Times Final: `{find_times_final}`
- Hit Update Called: `{hit_update_called}`
- Team GP Update Called: `{team_gp_update_called}`
- Team Search Info Update Called: `{team_search_info_update_called}`
- Circular Miss Update Applied: `{circular_miss_update_applied}`
...
""".format(
        ...
        hit_update_called=summary.get("hit_update_called"),
        team_gp_update_called=summary.get("team_gp_update_called"),
        team_search_info_update_called=summary.get("team_search_info_update_called"),
        circular_miss_update_applied=summary.get("circular_miss_update_applied"),
    )
...
def run_runtime() -> Dict[str, Any]:
    ...
    for event in events:
        if event.get("event_name") == "detection_step_sensor_adapter_event":
            detection_event = event
            break
    if detection_event is None:
        raise ValueError("No detection_step_sensor_adapter_event found")

    rt = _runtime_imports()
    state = _initialize_state(config, rt)
    controlled_runtime = _configure_controlled_runtime_state(state, config, rt)
    runtime_trace = _runtime_update_from_sensor_event(state, detection_event, config, rt)
    runtime_trace["controlled_runtime"] = controlled_runtime
    runtime_trace["runtime_python_executable"] = sys.executable
    runtime_trace["runtime_python_version"] = sys.version
    runtime_trace["runtime_wall_time_s"] = float(time.perf_counter() - started)
    ...
    summary = {
        ...
        "actor_truth_used_for_detection": False,
        "target_truth_used_for_detection": False,
        "sensor_detected_target": bool(detection_event.get("sensor_detected_target", False)),
        "rgb_has_target_signature": bool(detection_event.get("rgb_has_target_signature", False)),
        "rangefinder_hit": bool(detection_event.get("rangefinder_hit", False)),
        "rangefinder_min_positive_range_m": detection_event.get("rangefinder_min_positive_range_m"),
        "found_trigger_rule": detection_event.get("found_trigger_rule"),
        ...
        "new_found_indices": runtime_trace.get("new_found_indices"),
        "hit_branch_covered": bool(runtime_trace.get("hit_update_called", False)),
        "hit_update_called": bool(runtime_trace.get("hit_update_called", False)),
        "team_gp_update_called": bool(runtime_trace.get("team_gp_update_called", False)),
        "team_search_info_update_called": bool(runtime_trace.get("team_search_info_update_called", False)),
        "team_miss_update_called": bool(runtime_trace.get("team_miss_update_called", False)),
        "circular_miss_update_applied": False,
        "circular_miss_update_skipped_due_to_camera_fov": True,
        "remaining_intensity_mass_final": float(rt["remaining_intensity_mass"](state["intensity_map"])),
        "peak_intensity_ratio_final": float(rt["peak_intensity_ratio"](state["intensity_map"], state["nav_map_prior"])),
        "known_free_observation_ratio_final": float(state["known_free_observation_ratio_curve"][-1]),
        "gp_n_obs_initial": runtime_trace.get("initial_gp_n_obs"),
        "gp_n_obs_final": runtime_trace.get("final_gp_n_obs"),
        "gp_count_consistent_final": runtime_trace.get("final_gp_count_consistent"),
        "search_info_shape_ok_final": runtime_trace.get("final_search_info_shape_ok"),
        "search_info_valid_final": runtime_trace.get("final_search_info_valid"),
    ...
    _save_json(RUNTIME_UPDATE_JSON, runtime_trace)
    ...
    print(
        "Phase 5C-4B runtime finished: reason={0}, found={1}, hit_update={2}".format(
            summary["terminated_reason"],
            summary["found_count_final"],
            summary["hit_update_called"],
        )
    )
    return summary
