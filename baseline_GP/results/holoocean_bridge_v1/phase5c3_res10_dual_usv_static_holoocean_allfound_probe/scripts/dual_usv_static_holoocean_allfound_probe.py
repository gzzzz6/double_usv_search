"""Phase 5C-3: dual-USV static-target HoloOcean all_found probe.

This phase extends the Phase 5C-2 physical bridge until the fixed static
target is found and the team state terminates with all_found.
"""
...
def _phase_config() -> Dict[str, Any]:
    return {
        ...
        "scenario_sensor_types": ["GPSSensor", "LocationSensor", "OrientationSensor"],
...
def _make_scenario_config(spec: Dict[str, Any], start_world: Dict[int, List[float]]) -> Dict[str, Any]:
    ...
    return {
        "name": "dual_usv_static_holoocean_allfound_probe",
        "world": spec["world"],
...
def _build_summary_md(summary: Dict[str, Any]) -> str:
    return """# Phase 5C-3 HoloOcean All Found Probe Summary
...
def _generate_paths_png(nav_map_prior: np.ndarray, policy_trace: List[Dict[str, Any]], tick_trace: List[Dict[str, Any]]) -> None:
    try:
        ...
        plt.title("Phase 5C-3 HoloOcean Dual-USV Static All Found Probe")
        plt.xlabel("Column")
        plt.ylabel("Row")
        plt.xlim(-0.5, 80.5)
        plt.ylim(80.5, -0.5)
...
def main() -> None:
    print("=== Phase 5C-3: dual-USV static HoloOcean all_found probe ===")
    result = run_smoke()
    summary = result["summary"]
    print(
        "Phase 5C-3 all_found probe finished: reason={0}, policy_steps={1}, ticks={2}, found={3}".format(
