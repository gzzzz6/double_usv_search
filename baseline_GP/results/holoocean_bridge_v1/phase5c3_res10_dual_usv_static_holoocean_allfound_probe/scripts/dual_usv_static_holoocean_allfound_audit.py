"""Audit for Phase 5C-3 dual-USV static HoloOcean all_found probe."""
...
SMOKE_SCRIPT = os.path.normpath(os.path.join(PHASE_DIR, "scripts", "dual_usv_static_holoocean_allfound_probe.py"))
AUDIT_SCRIPT = os.path.normpath(os.path.join(PHASE_DIR, "scripts", "dual_usv_static_holoocean_allfound_audit.py"))
CONFIG_JSON = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "dual_usv_static_holoocean_allfound_config.json"))
POLICY_TRACE_JSON = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "dual_usv_static_holoocean_allfound_policy_trace.json"))
POLICY_TRACE_CSV = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "dual_usv_static_holoocean_allfound_policy_trace.csv"))
TICK_TRACE_JSON = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "dual_usv_static_holoocean_allfound_tick_trace.json"))
TICK_TRACE_CSV = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "dual_usv_static_holoocean_allfound_tick_trace.csv"))
FOUND_JSON = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "dual_usv_static_holoocean_allfound_found_events.json"))
SENSOR_JSON = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "dual_usv_static_holoocean_allfound_sensor_sources.json"))
COLLISION_JSON = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "dual_usv_static_holoocean_allfound_collision_metrics.json"))
SUMMARY_JSON = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "dual_usv_static_holoocean_allfound_summary.json"))
GIT_STATUS_JSON = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "dual_usv_static_holoocean_allfound_git_status.json"))
SUMMARY_MD = os.path.normpath(os.path.join(PHASE_DIR, "reports", "dual_usv_static_holoocean_allfound_summary.md"))
AUDIT_JSON = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "dual_usv_static_holoocean_allfound_audit.json"))
AUDIT_MD = os.path.normpath(os.path.join(PHASE_DIR, "reports", "dual_usv_static_holoocean_allfound_audit_summary.md"))
PATHS_PNG = os.path.normpath(os.path.join(PHASE_DIR, "visuals", "dual_usv_static_holoocean_allfound_paths.png"))

EXPECTED_MAX_POLICY_STEPS = 40
...
def run_audit() -> Dict[str, Any]:
    ...
    checks["max_policy_steps_is_40"] = int(config.get("max_policy_steps", -1)) == EXPECTED_MAX_POLICY_STEPS
    checks["expected_runtime_all_found_step_is_32"] = (
        int(config.get("expected_runtime_all_found_step", -1)) == EXPECTED_RUNTIME_ALL_FOUND_STEP
    )
    checks["expected_initial_target_positions_match"] = (
        config.get("expected_initial_target_positions") == EXPECTED_INITIAL_TARGET_POSITIONS
    )
    ...
    max_policy_steps = int(config.get("max_policy_steps", EXPECTED_MAX_POLICY_STEPS))
    ...
    checks["found_json_matches_summary_reason"] = found.get("terminated_reason") == summary.get("terminated_reason")
    checks["initial_target_positions_match_expected"] = found.get("initial_target_positions") == EXPECTED_INITIAL_TARGET_POSITIONS
    checks["found_expected_target_positions_recorded"] = (
        found.get("expected_initial_target_positions") == EXPECTED_INITIAL_TARGET_POSITIONS
    )
    checks["found_expected_runtime_step_recorded"] = (
        int(found.get("expected_runtime_all_found_step", -1)) == EXPECTED_RUNTIME_ALL_FOUND_STEP
    )
    checks["terminated_reason_all_found"] = summary.get("terminated_reason") == "all_found" and found.get("terminated_reason") == "all_found"
    checks["completed_policy_steps_expected_32"] = (
        int(summary.get("completed_policy_steps", -1)) == EXPECTED_RUNTIME_ALL_FOUND_STEP
        and int(found.get("completed_policy_steps", -1)) == EXPECTED_RUNTIME_ALL_FOUND_STEP
        and len(policy_trace) == EXPECTED_RUNTIME_ALL_FOUND_STEP
    )
    checks["found_count_final_is_1"] = int(found.get("found_count_final", -1)) == 1
    checks["found_mask_final_true"] = found.get("found_mask_final") == [True]
    checks["find_times_final_32"] = found.get("find_times_final") == [EXPECTED_RUNTIME_ALL_FOUND_STEP]
    checks["hit_branch_covered_required"] = bool(found.get("hit_branch_covered", False)) and bool(summary.get("hit_branch_covered", False))
    checks["remaining_intensity_mass_final_zero"] = (
        _is_finite_number(found.get("remaining_intensity_mass_final"))
        and float(found.get("remaining_intensity_mass_final")) <= 1e-9
    )
    checks["peak_intensity_ratio_final_zero"] = (
        _is_finite_number(found.get("peak_intensity_ratio_final"))
        and float(found.get("peak_intensity_ratio_final")) <= 1e-9
    )
    expected_row = None
    for row in policy_trace:
        if int(row.get("step", -1)) == EXPECTED_RUNTIME_ALL_FOUND_STEP:
            expected_row = row
            break
    checks["expected_step_row_present"] = expected_row is not None
    checks["expected_step_detected_target"] = (
        expected_row is not None
        and int(expected_row.get("team_detected_count", 0)) == 1
        and expected_row.get("new_found_indices") == [0]
        and bool(expected_row.get("hit_update_called", False))
        and expected_row.get("terminated_reason_after_step") == "all_found"
    )
    found_details = found.get("found_event_details", [])
    checks["found_event_details_present"] = isinstance(found_details, list) and len(found_details) == 1
    if isinstance(found_details, list) and len(found_details) == 1:
        detail = found_details[0]
        checks["found_event_details_complete"] = (
            int(detail.get("target_index", -1)) == 0
            and int(detail.get("found_step", -1)) == EXPECTED_RUNTIME_ALL_FOUND_STEP
            and detail.get("target_position") == EXPECTED_INITIAL_TARGET_POSITIONS[0]
            and isinstance(detail.get("detected_usvs"), list)
            and len(detail.get("detected_usvs")) >= 1
            and isinstance(detail.get("usv_projected_cells"), dict)
            and len(detail.get("usv_projected_cells")) >= 1
        )
    else:
        checks["found_event_details_complete"] = False
    ...
    metrics = {
        ...
        "found_mask_final": found.get("found_mask_final"),
        "find_times_final": found.get("find_times_final"),
        ...
        "remaining_intensity_mass_final": found.get("remaining_intensity_mass_final"),
        "peak_intensity_ratio_final": found.get("peak_intensity_ratio_final"),
        "gp_n_obs_initial": found.get("gp_n_obs_initial"),
        "gp_n_obs_final": found.get("gp_n_obs_final"),
        "found_event_details": found.get("found_event_details"),
