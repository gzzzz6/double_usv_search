"""Audit for Phase 5D-8b live planner callback continuous search."""
...
PROBE_SCRIPT = os.path.join(PHASE_DIR, "scripts", "live_planner_callback_scan_capture_search_probe.py")
AUDIT_SCRIPT = os.path.join(PHASE_DIR, "scripts", "live_planner_callback_scan_capture_search_audit.py")
...
AUDIT_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d8b_audit.json")
AUDIT_MD = os.path.join(PHASE_DIR, "reports", "phase5d8b_audit_summary.md")
...
def _build_md(checks: Dict[str, bool], metrics: Dict[str, Any]) -> str:
    lines = [
        "# Phase 5D-8b Live Planner Callback Continuous Search Audit Summary",
        "",
