"""HoloOcean Phase 4B: 10m OpenWater fast waypoint controller smoke.

Evaluates a customized proportional waypoint controller using twin propeller thrust on a 10m grid.
"""
...
def generate_preview_map(
    ...
    try:
        ...
        plt.title(
            f"Phase 4B 10m fast waypoint controller smoke\n"
            f"(openwater_open_res10_v1, 81x81, cell=10m, scheme=0)",
            fontsize=12,
            pad=10,
        )
...
def write_summary_report(
    ...
    header = f"""# Phase 4B: SurfaceVessel control_scheme=0 Fast Controller Smoke Report (10m)
...
def main() -> None:
    print("=== Phase 4B: Launching 10m Fast Waypoint Controller Smoke ===")
    ...
    # Roundtrip check to satisfy rule 6
    for idx, tc in enumerate([start_cell] + waypoints_grid):
        w_pt = grid_to_world(tc, config=adapter_config)
        g_pt = world_to_grid(w_pt, config=adapter_config, map_shape=nav_map_prior.shape)
        assert g_pt == tc, f"Roundtrip check failed at index {idx}: grid={tc} -> world={w_pt} -> grid={g_pt}"
    print("Bi-directional coordinate roundtrip checks passed for all cells.")

    scenario_cfg = {
        "name": "fast_controller_res10_smoke",
        "world": spec["world"],
    ...
    with holoocean.make(scenario_cfg=scenario_cfg) as env:
        ...
        for wp_idx, target_cell in enumerate(waypoints_grid):
            ...
            while not arrived and not timeout:
                ...
                loc_sv = get_sensor_vector(state, "sv", "LocationSensor")
                gps_sv = get_sensor_vector(state, "sv", "GPSSensor")
                orient_sv = get_sensor_vector(state, "sv", "OrientationSensor")
                ...
                # -----------------------------------------------------------
                # Proportional steering and cos-weighted thrust
                # -----------------------------------------------------------
                ...
                # Cos-weighted forward factor to rotate in place when heading error is high
    ...
    print(f"\n==========================================")
    print(f"10m Fast Controller Smoke Completed!")
