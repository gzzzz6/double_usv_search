"""Phase 5D-13: live planner callback continuous HoloOcean search.
...
Compared with 5D-9, this phase adds a planner-segment lookahead standoff hold
in the HoloOcean bridge wrapper. The hold changes only live sensor sampling
pose; baseline_GP planner decisions, target truth, and the detection rule are
unchanged.
...
PHASE_NAME = "phase5d13_live_rgb_range_sync_fix"
...
CONFIG_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d13_config.json")
PRODUCTION_CONTRACT_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d13_production_adapter_contract.json")
CALIBRATION_RUN_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d13_live_calibration_run.json")
TARGET_RUN_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d13_live_target_run.json")
TEAMMATE_RUN_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d13_live_teammate_only_run.json")
TICK_TRACE_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d13_live_tick_trace.json")
TICK_TRACE_CSV = os.path.join(PHASE_DIR, "manifests", "phase5d13_live_tick_trace.csv")
POLICY_TRACE_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d13_live_policy_trace.json")
POLICY_TRACE_CSV = os.path.join(PHASE_DIR, "manifests", "phase5d13_live_policy_trace.csv")
EVENTS_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d13_live_per_agent_detection_events.json")
EVENTS_CSV = os.path.join(PHASE_DIR, "manifests", "phase5d13_live_per_agent_detection_events.csv")
FUSION_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d13_live_candidate_fusion_trace.json")
FUSION_CSV = os.path.join(PHASE_DIR, "manifests", "phase5d13_live_candidate_fusion_trace.csv")
ADAPTER_TRACE_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d13_adapter_observe_trace.json")
ADAPTER_TRACE_CSV = os.path.join(PHASE_DIR, "manifests", "phase5d13_adapter_observe_trace.csv")
MAINLINE_UPDATE_TRACE_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d13_mainline_update_found_trace.json")
MAINLINE_UPDATE_TRACE_CSV = os.path.join(PHASE_DIR, "manifests", "phase5d13_mainline_update_found_trace.csv")
SUMMARY_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d13_summary.json")
GIT_STATUS_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d13_git_status.json")
RAW_FRAME_SYNC_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d13_raw_frame_sync_diagnostic.json")
RAW_FRAME_SYNC_CSV = os.path.join(PHASE_DIR, "manifests", "phase5d13_raw_frame_sync_diagnostic.csv")
SUMMARY_MD = os.path.join(PHASE_DIR, "reports", "phase5d13_summary.md")
BASE_COMPILE_LOG = os.path.join(PHASE_DIR, "logs", "phase5d13_py_compile_base.txt")
HOLO_COMPILE_LOG = os.path.join(PHASE_DIR, "logs", "phase5d13_py_compile_holo.txt")
...
RGB_SYNC_STRATEGY = "short_window_detector_scored_nearest_pair"
...
P5D3 = _load_module(SOURCE_5D3_PROBE, "phase5d13_p5d3_detection_source")
...
def _phase_config(pre_git_status: List[str]) -> Dict[str, Any]:
    return {
        ...
        "phase_goal": "Run baseline_GP mainline planner callback with live HoloOcean movement/capture, planner-segment standoff sensing hold, and production adapter driven update_found_mask/all_found.",
        ...
        "rgb_sync_scope": "same_policy_step_target_and_calibration_rgb_pair_only_rangefinder_tick_keeps_range_and_pose",
        ...
        "target_placement_source": "phase5d13_fixed_openwater_cell_on_baseline_planner_forward_fan_path",
        ...
        "post_arrival_stabilization_role": "zero_thrust_live_capture_standoff_stabilization_only_baseline_grid_state_unchanged",
        ...
        "standoff_viewpoint_role": "holoocean_bridge_sensor_pose_hold_only_core_planner_unchanged",
        ...
        "post_arrival_scan_role": "holoocean_bridge_live_sensor_sampling_pose_only_core_planner_unchanged",
        "gp_fit_every": 5,
        "gp_max_points": 48,
...
def _scenario_config(
    *,
    ...
    return {
        "name": "phase5d13_{0}".format(run_kind),
...
def _select_synced_rgb_pair(
    *,
    ...
    target_by_step = target_run.get("raw_captures_by_step_tick", {}).get(int(step), {})
    baseline_by_step = calibration_reference.get("raw_captures_by_step_tick", {}).get(int(step), {})
    if not isinstance(target_by_step, dict) or not isinstance(baseline_by_step, dict):
        result["rgb_sync_failure_reason"] = "missing_step_capture_window"
        return result

    candidates: List[Dict[str, Any]] = []
    for target_tick_key, target_by_agent in target_by_step.items():
        try:
            target_tick = int(target_tick_key)
        except Exception:
            continue
        target_offset = target_tick - int(tick_in_step)
        if abs(target_offset) > int(RGB_SYNC_WINDOW_TICKS) or not isinstance(target_by_agent, dict):
            continue
        target_capture = target_by_agent.get(agent_name)
        if not isinstance(target_capture, dict) or target_capture.get("rgb") is None:
            continue
        for baseline_tick_key, baseline_by_agent in baseline_by_step.items():
            try:
                baseline_tick = int(baseline_tick_key)
            except Exception:
                continue
            baseline_offset = baseline_tick - int(tick_in_step)
            if abs(baseline_offset) > int(RGB_SYNC_WINDOW_TICKS) or not isinstance(baseline_by_agent, dict):
                continue
            baseline_capture = baseline_by_agent.get(agent_name)
            if not isinstance(baseline_capture, dict) or baseline_capture.get("rgb") is None:
                continue
            ...
            target_pose_drift = _distance_2d(
                ...
                target_by_step.get(int(tick_in_step), {}).get(agent_name, {}).get("observer_world_location")
                if isinstance(target_by_step.get(int(tick_in_step), {}), dict)
                else None,
            )
            baseline_pose_drift = _distance_2d(
                baseline_capture.get("observer_world_location"),
                baseline_by_step.get(int(tick_in_step), {}).get(agent_name, {}).get("observer_world_location")
                if isinstance(baseline_by_step.get(int(tick_in_step), {}), dict)
                else None,
            )
            ...
            candidates.append(
                {
                    ...
                    "baseline_tick": int(baseline_tick),
    ...
    *,
    ...
    if display_live and run_kind == "target":
        live_viewer = LiveSearchViewer(
            ...
            map_shape=nav_map_prior.shape,
            title="Phase 5D-13 Live Dual-USV Search",
            fps=display_fps,
    ...
    adapter = ReusableMainlinePerceptionAdapter(adapter_id="phase5d13_{0}".format(run_kind))
    adapter.reset_episode("phase5d13_{0}".format(run_kind))
    launch_ok = False
    error = ""
...
def _raw_frame_case_row(
    *,
    ...
    if event.get("rgb_range_sync_applied") is True:
        synced_target_tick = event.get("rgb_sync_target_tick")
        synced_baseline_tick = event.get("rgb_sync_baseline_tick")
        if synced_target_tick is not None:
            synced_target_capture = _capture_at(target_run, step, int(synced_target_tick), agent_name)
        if synced_baseline_tick is not None:
            synced_baseline_capture = _capture_at(calibration_run, step, int(synced_baseline_tick), agent_name)
    target_rgb = (
    ...
    base = os.path.join(
        PHASE_DIR,
        "visuals",
        "phase5d13_step{0:02d}_tick{1:03d}_{2}".format(step, tick, agent_name),
    )
    if save_visuals:
        visuals = {
...
def _build_raw_frame_sync_diagnostic(
    *,
    ...
    return {
        "phase_name": PHASE_NAME,
        "diagnostic_name": "phase5d13_raw_frame_baseline_sync_diagnostic",
...
def _run_compile_log(path: str) -> None:
    ...
    result = subprocess.run(
        [
            ...
            "-m",
            "py_compile",
            os.path.join(PHASE_DIR, "scripts", "live_rgb_range_sync_fix_probe.py"),
        ],
        cwd=os.path.abspath("."),
        check=False,
        text=True,
...
def _build_summary(
    *,
    ...
    phase_status = (
        "live_planner_callback_standoff_viewpoint_all_found_closed"
...
def _build_summary_md(summary: Dict[str, Any]) -> str:
    lines = [
        ...
        (
            ...
            else "Phase 5D-13 is a raw-frame diagnostic: it exercises the real planner callback and live HoloOcean capture path, saves selected target/baseline RGB evidence, and does not by itself claim the HoloOcean real search all_found chain is closed."
        ),
        "",
    ]
    return "\n".join(lines)
...
def run_live_planner_callback_search(
    *,
    ...
    print("Running 5D-13 live calibration/reference planner callback run")
    ...
    print("Running 5D-13 live target planner callback run")
    ...
    print("Running 5D-13 live teammate-only planner callback run")
