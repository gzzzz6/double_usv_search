"""Phase 5D-1: SurfaceVessel teammate exclusion validation.
...
after capture to audit whether teammate-only scenes produce false positives.
"""
...
TARGET_Z = 0.5
TEAMMATE_Z = 0.0
TARGET_DISTANCES_M = [20.0, 35.0]
TEAMMATE_SINGLE_DISTANCES_M = [10.0, 15.0, 20.0, 35.0]
TEAMMATE_EDGE_DISTANCES_M = [15.0, 35.0]
FAN_TEST_ANGLES = [
...
def _build_scenes() -> List[Dict[str, Any]]:
    ...
    multi_specs = [
        {
            "name": "teammate_only_surfacevessel_pair_inner_20m",
            "role": "surfacevessel_multi_teammate_negative",
            "teammates": [
                _object_spec(TEAMMATE_NEGATIVE_AGENT_TYPE, observer_world, 20.0, 15.0, TEAMMATE_Z),
                _object_spec(TEAMMATE_NEGATIVE_AGENT_TYPE, observer_world, 20.0, -15.0, TEAMMATE_Z),
            ],
            "distance_m": 20.0,
        ...
        {
            "name": "teammate_only_surfacevessel_pair_outer_35m",
            "role": "surfacevessel_multi_teammate_negative",
            "teammates": [
                _object_spec(TEAMMATE_NEGATIVE_AGENT_TYPE, observer_world, 35.0, 25.0, TEAMMATE_Z),
                _object_spec(TEAMMATE_NEGATIVE_AGENT_TYPE, observer_world, 35.0, -25.0, TEAMMATE_Z),
            ],
            "distance_m": 35.0,
        ...
        {
            "name": "teammate_only_surfacevessel_triple_fan_20m",
            "role": "surfacevessel_multi_teammate_negative",
            "teammates": [
                _object_spec(TEAMMATE_NEGATIVE_AGENT_TYPE, observer_world, 20.0, 0.0, TEAMMATE_Z),
                _object_spec(TEAMMATE_NEGATIVE_AGENT_TYPE, observer_world, 22.0, 20.0, TEAMMATE_Z),
                _object_spec(TEAMMATE_NEGATIVE_AGENT_TYPE, observer_world, 22.0, -20.0, TEAMMATE_Z),
            ],
            "distance_m": 20.0,
            "angle_label": "center_plus_inner_pair",
            "angle_deg": None,
        },
    ]
...
def _phase_config(mode: str, pre_git_status: Optional[List[str]] = None, recommended_rule: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "mode": mode,
        "purpose": "SurfaceVessel teammate exclusion stress validation for the fixed 5D-0 SphereAgent RGB/RangeFinder rule",
...
def _scenario_config(scene: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "name": "phase5d1_{0}_{1}".format(config.get("mode", "capture"), scene["name"]),
        "world": "OpenWater",
        "package_name": "Ocean",
        "main_agent": "sv0",
        "agents": agents,
...
def _fixed_recommended_rule(summary_rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    fixed["recommendation_reason"] = (
        "Phase 5D-1 validates the Phase 5D-0 fixed SphereAgent sphere-blob + fan-RangeFinder rule "
        "against expanded SurfaceVessel teammate-only and coexistence scenes; it does not reselect the detector."
    )
...
def run_calibrate() -> Dict[str, Any]:
    ...
    with open(CAL_SUMMARY_MD, "w", encoding="utf-8") as f:
        f.write(_build_summary_md(summary, "Phase 5D-1 Calibration Summary"))
    ...
    print(
        "Phase 5D-1 calibration finished: recommended={0}, reliable={1}, teammate_fp={2}".format(
...
def run_validate() -> Dict[str, Any]:
    ...
    with open(VAL_SUMMARY_MD, "w", encoding="utf-8") as f:
        f.write(_build_summary_md(summary, "Phase 5D-1 HoloOcean Validation Summary"))
    post_git_status = _run_git_status()
    ...
    print(
        "Phase 5D-1 validation finished: target={0}, teammate={1}, reliable={2}".format(
