...
# 1. Install PEP 563 annotations compatibility finder for Python 3.8 conda environment
if sys.version_info < (3, 9):
    import importlib.abc
    import importlib.machinery

    class Python38AnnotationCompatFinder(importlib.abc.MetaPathFinder):
        def find_spec(self, fullname, path, target=None):
            if not fullname.startswith("baseline_GP"):
                return None
            ...
            if isinstance(spec.loader, importlib.machinery.SourceFileLoader):
                spec.loader = Python38SourceLoader(spec.loader.name, spec.loader.path)
            return spec

    class Python38SourceLoader(importlib.machinery.SourceFileLoader):
        def get_code(self, fullname):
            ...
            patched_code = "from __future__ import annotations\n" + cleaned_code
            return compile(patched_code, self.path, "exec")

    sys.meta_path.insert(0, Python38AnnotationCompatFinder())
...
# Import restructured bridge adapters and builders
...
def generate_trajectory_preview(
    ...
    try:
        ...
        grid_rows = [1] + [d["final_projected_cell"][0] for d in decisions]
        grid_cols = [1] + [d["final_projected_cell"][1] for d in decisions]

        target_rows = [d["target_cell"][0] for d in decisions]
        target_cols = [d["target_cell"][1] for d in decisions]
        ...
        plt.title("Phase 3A: Closed-Loop Single-USV Search Algorithm Step Integration\n(openwater_open_v1, shape=81x81, cell_size=5m, start=[1,1])", fontsize=11, pad=10)
...
def write_summary_report(
    ...
    header = f"""# HoloOcean Algorithm Step Smoke Test Report - Phase 3A
...
- **Waypoints Successfully Arrived**: `{sum(1 for d in decisions if d["arrived"]):.0f} / {len(decisions)}`
...
"""
    ...
    for d in decisions:
        ...
        target_cell_str = f"({d['target_cell'][0]}, {d['target_cell'][1]})"
        target_world_str = f"({d['target_world'][0]:.1f}, {d['target_world'][1]:.1f})"
        final_world_str = f"({d['final_world'][0]:.2f}, {d['final_world'][1]:.2f})"
        final_grid_str = f"({d['final_projected_cell'][0]}, {d['final_projected_cell'][1]})"
        
        arrived_str = "`PASSED`" if d["arrived"] else "`FAILED`"
        audit_str = "`PASSED`" if d["chebyshev_audit"] else "`FAILED`"
        replan_str = f"`{d['replan_reason']}`" if d["replanned"] else "`NONE`"

        rows.append(
            f"| {d['step']} | {pos_before_str} | {target_cell_str} | {target_world_str} | {final_world_str} | {final_grid_str} | {d['ticks']} | {arrived_str} | {audit_str} | {replan_str} |"
        )
...
def main():
    print("=== Phase 3A: Launching Closed-loop single-USV search policy integration ===")
    ...
    with holoocean.make(scenario_cfg=scenario_cfg) as env:
        for step in range(1, 9):  # Run exactly 8 closed-loop steps
            ...
            # 3-Fold Coordinate assertions (rigid checkpoint)
            ...
            decision_record = {
                ...
                "arrived": step_summary["arrived"],
                "chebyshev_audit": chebyshev_audit_passed,
                "replanned": state.get("_audit_replanned", False),
    ...
    print("Phase 3A Smoke execution complete!")
