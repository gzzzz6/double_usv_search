"""Phase 5D-0: static SphereAgent target calibration.

This phase starts the "heterogeneous static target" line:

    observer: SurfaceVessel
    target:   static SphereAgent
    negative: SurfaceVessel teammate-like actor

It does not use actor truth to trigger detection. Truth labels are used only
after capture to audit candidate rules.
"""
            ...
            scenes.append(
                {
                    ...
                    "target": None,
                    "teammate": _object_spec(TEAMMATE_NEGATIVE_AGENT_TYPE, observer_world, distance, angle_deg, TEAMMATE_Z),
                    "include_target": False,
...
def _phase_config(mode: str, pre_git_status: Optional[List[str]] = None, recommended_rule: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "mode": mode,
        "purpose": "single-observer static SphereAgent RGB/RangeFinder calibration with SurfaceVessel teammate-only negatives",
        ...
        "planned_scene_counts": {
            ...
            "teammate_only": len(DISTANCES_M) * len(FAN_TEST_ANGLES),
        },
        "scenes": scenes,
        "map_npz": MAP_NPZ,
...
def _scenario_config(scene: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "name": "phase5d0_{0}_{1}".format(config.get("mode", "capture"), scene["name"]),
        "world": "OpenWater",
        "package_name": "Ocean",
        "main_agent": "sv0",
        "agents": agents,
    ...
    try:
        with holoocean.make(scenario_cfg=_scenario_config(scene, config)) as env:
            ...
            for tick in range(1, CAPTURE_TICKS + 1):
                ...
                if bool(scene.get("include_teammate", False)):
                    env.act("teammate", _zero_action(TEAMMATE_NEGATIVE_AGENT_TYPE))
                state = env.tick()
...
def _summarize_rule(rule: Dict[str, Any], rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    fan_reliable = []
    for distance in DISTANCES_M:
        rows_at_distance = [row for row in target_rows if float(row.get("distance_m")) == float(distance)]
    ...
    return {
        **rule,
        ...
        "target_50m_true_positive_count": _hits(50.0),
        "target_50m_total": _total(50.0),
...
def _summary_metrics(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for angle in FAN_TEST_ANGLES:
        label = str(angle["label"])
        rows_at_angle = [row for row in target_rows if row.get("angle_label") == label]
...
def run_calibrate() -> Dict[str, Any]:
    ...
    recommended = _recommend_rule(candidate_summaries)
    ...
    with open(CAL_SUMMARY_MD, "w", encoding="utf-8") as f:
        f.write(_build_summary_md(summary, "Phase 5D-0 Calibration Summary"))
    ...
    print(
        "Phase 5D-0 calibration finished: recommended={0}, reliable={1}, teammate_fp={2}".format(
...
def run_validate() -> Dict[str, Any]:
    ...
    with open(VAL_SUMMARY_MD, "w", encoding="utf-8") as f:
        f.write(_build_summary_md(summary, "Phase 5D-0 HoloOcean Validation Summary"))
    post_git_status = _run_git_status()
    ...
    print(
        "Phase 5D-0 validation finished: target={0}, teammate={1}, reliable={2}".format(
