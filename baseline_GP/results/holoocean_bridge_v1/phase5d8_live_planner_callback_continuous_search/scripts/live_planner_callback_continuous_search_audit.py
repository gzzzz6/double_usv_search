"""Audit for Phase 5D-8 live planner callback continuous search."""
...
PROBE_SCRIPT = os.path.join(PHASE_DIR, "scripts", "live_planner_callback_continuous_search_probe.py")
AUDIT_SCRIPT = os.path.join(PHASE_DIR, "scripts", "live_planner_callback_continuous_search_audit.py")
...
AUDIT_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d8_audit.json")
AUDIT_MD = os.path.join(PHASE_DIR, "reports", "phase5d8_audit_summary.md")
...
def _build_md(checks: Dict[str, bool], metrics: Dict[str, Any]) -> str:
    lines = [
        "# Phase 5D-8 Live Planner Callback Continuous Search Audit Summary",
        "",
...
def main() -> None:
    ...
    checks["phase_completion_fields_consistent"] = (
        ...
        and (
            (phase_goal_completed and summary.get("phase_status") == "live_planner_callback_all_found_closed")
            or (
