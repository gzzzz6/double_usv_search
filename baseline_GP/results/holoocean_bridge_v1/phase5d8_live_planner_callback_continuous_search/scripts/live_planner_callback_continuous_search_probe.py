"""Phase 5D-8: live planner callback continuous HoloOcean search.
...
PHASE_NAME = "phase5d8_live_planner_callback_continuous_search"
...
CONFIG_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d8_config.json")
PRODUCTION_CONTRACT_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d8_production_adapter_contract.json")
CALIBRATION_RUN_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d8_live_calibration_run.json")
TARGET_RUN_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d8_live_target_run.json")
TEAMMATE_RUN_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d8_live_teammate_only_run.json")
TICK_TRACE_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d8_live_tick_trace.json")
TICK_TRACE_CSV = os.path.join(PHASE_DIR, "manifests", "phase5d8_live_tick_trace.csv")
POLICY_TRACE_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d8_live_policy_trace.json")
POLICY_TRACE_CSV = os.path.join(PHASE_DIR, "manifests", "phase5d8_live_policy_trace.csv")
EVENTS_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d8_live_per_agent_detection_events.json")
EVENTS_CSV = os.path.join(PHASE_DIR, "manifests", "phase5d8_live_per_agent_detection_events.csv")
FUSION_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d8_live_candidate_fusion_trace.json")
FUSION_CSV = os.path.join(PHASE_DIR, "manifests", "phase5d8_live_candidate_fusion_trace.csv")
ADAPTER_TRACE_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d8_adapter_observe_trace.json")
ADAPTER_TRACE_CSV = os.path.join(PHASE_DIR, "manifests", "phase5d8_adapter_observe_trace.csv")
MAINLINE_UPDATE_TRACE_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d8_mainline_update_found_trace.json")
MAINLINE_UPDATE_TRACE_CSV = os.path.join(PHASE_DIR, "manifests", "phase5d8_mainline_update_found_trace.csv")
SUMMARY_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d8_summary.json")
GIT_STATUS_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d8_git_status.json")
SUMMARY_MD = os.path.join(PHASE_DIR, "reports", "phase5d8_summary.md")
BASE_COMPILE_LOG = os.path.join(PHASE_DIR, "logs", "phase5d8_py_compile_base.txt")
HOLO_COMPILE_LOG = os.path.join(PHASE_DIR, "logs", "phase5d8_py_compile_holo.txt")
...
P5D3 = _load_module(SOURCE_5D3_PROBE, "phase5d8_p5d3_detection_source")
...
def _phase_config(pre_git_status: List[str]) -> Dict[str, Any]:
    return {
        ...
        "target_placement_source": "phase5d8_fixed_openwater_cell_on_baseline_planner_forward_fan_path",
...
def _fan_rangefinder_names() -> List[str]:
    return [
        ...
        for index in range(len(P5D3.P5D1A.FAN_YAW_DEGREES))
    ]
...
def _scenario_config(
    *,
    ...
    return {
        "name": "phase5d8_{0}".format(run_kind),
...
def _drive_policy_step(
    *,
    ...
    def _act_and_capture(stabilization_tick: bool = False) -> None:
        ...
        for usv_id in (0, 1):
            ...
            if wait_applied_map[usv_id]:
                ...
                arrived[usv_id] = True
            raw_captures[step_tick_count][agent_name] = _capture_agent(state_env, agent_name, selected)
...
def _run_live_case(
    *,
    ...
    adapter = ReusableMainlinePerceptionAdapter(adapter_id="phase5d8_{0}".format(run_kind))
    adapter.reset_episode("phase5d8_{0}".format(run_kind))
    launch_ok = False
    error = ""
...
def _run_compile_log(path: str) -> None:
    ...
    result = subprocess.run(
        [
            ...
            "-m",
            "py_compile",
            os.path.join(PHASE_DIR, "scripts", "live_planner_callback_continuous_search_probe.py"),
            os.path.join(PHASE_DIR, "scripts", "live_planner_callback_continuous_search_audit.py"),
        ],
        cwd=os.path.abspath("."),
        check=False,
        text=True,
...
def _build_summary(
    *,
    ...
    phase_status = (
        "live_planner_callback_all_found_closed"
...
def _build_summary_md(summary: Dict[str, Any]) -> str:
    lines = [
        "# Phase 5D-8 Live Planner Callback Continuous Search Summary",
        "",
        ...
        "",
        (
            "Phase 5D-8 closes the static SphereAgent core migration path: baseline_GP chooses each step, HoloOcean executes and captures live sensors, the production adapter emits the mainline detected_mask, and the original update_found_mask/all_found chain terminates the search."
            if summary.get("phase_goal_completed") is True
            else "Phase 5D-8 exercised the real planner callback and live HoloOcean capture path, but did not close the all_found chain: live detection produced no accepted shared SphereAgent candidate under the planner-driven trajectory, so the original found_mask/all_found termination was not triggered."
        ),
        "",
    ]
    return "\n".join(lines)
...
def run_live_planner_callback_search() -> Dict[str, Any]:
    ...
    print("Running 5D-8 live calibration/reference planner callback run")
    ...
    print("Running 5D-8 live target planner callback run")
    ...
    print("Running 5D-8 live teammate-only planner callback run")
