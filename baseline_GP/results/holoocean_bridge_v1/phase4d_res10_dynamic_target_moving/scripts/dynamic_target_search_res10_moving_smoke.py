"""E2E Closed-loop dynamic target moving search verification – Phase 4D-moving: 10m OpenWater.

Runs E2E search policy decisions with A* and GP in HoloOcean 10m grid with a dynamic target agent crossing grid cells.
"""
...
MAX_STEPS = 120
TARGET_MOTION_TICKS_PER_STEP = 40  # 40 ticks for guaranteed cross-grid motion
...
def generate_route_map(
    ...
    try:
        ...
        if target_trace:
            ...
            plt.scatter(t_cols[0], t_rows[0], color="red", marker="X", s=150, zorder=5, label="Target Start (45,60)")
        ...
        plt.title(
            f"Phase 4D-moving: 10m Closed-loop Dynamic Target Moving Search Route Map\n"
...
def write_summary_report(
    ...
    header = f"""# Phase 4D-moving: E2E 10m OpenWater Cross-Grid Dynamic Target Search Summary Report
...
- **Unique Observed Target Cells Count**: `{unique_count}`
- **Target Observed Cell Changed**: `{changed}`
- **Target Cross-Grid Motion Valid**: `{valid}`
...
def main() -> None:
    print("=== Phase 4D-moving: Launching 10m Closed-Loop Cross-Grid Dynamic Target Search Smoke ===")
    ...
    scenario_cfg = {
        "name": "dynamic_target_search_res10_moving_smoke",
        "world": spec["world"],
    ...
    with holoocean.make(scenario_cfg=scenario_cfg) as env:
        ...
        for step in range(1, MAX_STEPS + 1):
            ...
            # Step B: Control target agent motion for TARGET_MOTION_TICKS_PER_STEP = 40 ticks
            ...
            # Check if observed cell changed from previous step
            observed_cell_changed = False
            if prev_observed_cell is not None and target_cell_observed != prev_observed_cell:
                observed_cell_changed = True
            prev_observed_cell = target_cell_observed

            # Calculate distance desired to observed
            dist_des_to_obs = float(math.hypot(target_x_cmd - last_target_x, target_y_cmd - last_target_y))
            ...
            t_trace_entry = {
                ...
                "observed_cell_changed_from_previous": observed_cell_changed,
                "distance_desired_to_observed_m": dist_des_to_obs,
                "target_position_source": "holoocean_agent_projection"
            }
            target_trace.append(t_trace_entry)

            print(f"\nStep {step:02d} -> Target Desired: {desired_cell} | Target Observed: {target_cell_observed} (Changed: {observed_cell_changed})")

            # Step F: Update clue field truth map with new target positions
            _update_clue_truth_map(state)

            # Step G: Call plan_next_policy_cell
            ...
            while not arrived and not timeout:
                ...
                # Act on BOTH and tick (target continues moving to its current desired cell command)
            ...
            # Step I: Verify Chebyshev distance
    ...
    print(f"\n==========================================")
    print(f"10m Cross-Grid Dynamic Target Search Smoke Completed!")
    ...
    # Calculate metrics
    observed_cells_seq = [tuple(entry["observed_cell"]) for entry in target_trace]
    unique_observed_cells = list(dict.fromkeys(observed_cells_seq))
    unique_count = len(unique_observed_cells)
    target_changed = any(entry["observed_cell_changed_from_previous"] for entry in target_trace)
    ...
    found_events = {
        ...
        "unique_observed_target_cells_count": unique_count,
        "target_cell_changed": target_changed,
        "target_cross_grid_motion_valid": valid_moving
    }
    ...
    write_summary_report(
        SUMMARY_MD, spec, state, decisions, target_trace, found_events, elapsed_time, total_ticks,
        unique_count, target_changed, valid_moving
    )
    generate_route_map(nav_map_prior, decisions, target_trace, found_step, PREVIEW_PNG)
    print("Phase 4D-moving cross-grid dynamic target search smoke complete!")
