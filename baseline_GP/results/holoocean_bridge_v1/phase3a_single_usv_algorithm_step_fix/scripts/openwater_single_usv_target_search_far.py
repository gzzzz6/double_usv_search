"""E2E Closed-loop integration smoke test – Phase 3B-repeat-far: Single-USV Target Search Far.

Runs single-USV search policy decisions in HoloOcean with a static target spawned at (45, 60).
...
DECISIONS_CSV     = os.path.join(PHASE_DIR, "manifests", "target_search_far_decisions.csv")
...
SUMMARY_MD        = os.path.join(PHASE_DIR, "reports",   "target_search_far_summary.md")
PREVIEW_PNG       = os.path.join(PHASE_DIR, "visuals",   "target_search_far_route_map.png")

MAX_STEPS = 250
RUN_LABEL = "target_search_far"
TARGET_ID = "suspicious_target_far_0"


# ---------------------------------------------------------------------------
# Helper: guard against overwriting existing results
# ---------------------------------------------------------------------------
_PROTECTED_FILES = {
...
# Protect Phase 3B-repeat-mid results
_PROTECTED_3B_MID = {
    os.path.abspath(os.path.join(PHASE_DIR, "manifests", "target_search_mid_decisions.json")),
    os.path.abspath(os.path.join(PHASE_DIR, "manifests", "target_search_mid_decisions.csv")),
    os.path.abspath(os.path.join(PHASE_DIR, "manifests", "target_search_mid_holoocean_tick_trace.csv")),
    os.path.abspath(os.path.join(PHASE_DIR, "manifests", "target_search_mid_policy_trace_rows.json")),
    os.path.abspath(os.path.join(PHASE_DIR, "manifests", "target_search_mid_target_manifest.json")),
    os.path.abspath(os.path.join(PHASE_DIR, "manifests", "target_search_mid_found_events.json")),
    os.path.abspath(os.path.join(PHASE_DIR, "reports",   "target_search_mid_summary.md")),
    os.path.abspath(os.path.join(PHASE_DIR, "visuals",   "target_search_mid_route_map.png")),
}
...
for _out in [TICK_CSV, DECISIONS_JSON, DECISIONS_CSV, POLICY_TRACE_JSON, TARGET_JSON, FOUND_JSON, SUMMARY_MD, PREVIEW_PNG]:
    ...
    assert os.path.abspath(_out) not in _PROTECTED_3B_MID, (
        f"PROTECTION VIOLATION: output path {_out!r} collides with Phase 3B-repeat-mid results."
    )
...
def generate_trajectory_preview(
    ...
    try:
        ...
        plt.scatter(target_cell[1], target_cell[0], color="gold", marker="^",
                    s=250, edgecolors="black", linewidths=1.5, zorder=8, label="Suspicious Target (45,60)")
        ...
        # Annotate every 15th step or first/last to keep the plot readable
        ...
        plt.title(
            f"Phase 3B-repeat-far OpenWater Target Search: baseline_GP decision + HoloOcean execution\n"
...
def write_summary_report(
    ...
    header = f"""# Phase 3B-repeat-far: Single-USV Target Search Closed Loop Report
...
def main() -> None:
    print(f"=== Phase 3B-repeat-far: Single-USV target search closed loop ===")
    ...
    # 2. Map target_cell (45, 60) to world coordinates and verify roundtrip
    target_cell = (45, 60)
    ...
    print("Belief initialization sync completed for target at (45,60).")
    ...
    with holoocean.make(scenario_cfg=scenario_cfg) as env:
        ...
        for step in range(1, MAX_STEPS + 1):
            ...
            decision_record: Dict[str, Any] = {
                ...
                "clue_mode_note":                    "clue_acquisition_mode = ucb temporary for Phase 3B-repeat-far",
                # Position / path
    ...
    print("Phase 3B-repeat-far smoke complete!")
