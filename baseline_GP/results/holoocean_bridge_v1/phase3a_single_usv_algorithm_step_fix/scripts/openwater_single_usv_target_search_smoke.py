"""E2E Closed-loop integration smoke test – Phase 3B-min: Single-USV Target Search.

Runs single-USV search policy decisions in HoloOcean with a static target spawned at (2, 30).
...
DECISIONS_CSV     = os.path.join(PHASE_DIR, "manifests", "target_search_decisions.csv")
...
SUMMARY_MD        = os.path.join(PHASE_DIR, "reports",   "target_search_summary.md")
PREVIEW_PNG       = os.path.join(PHASE_DIR, "visuals",   "target_search_route_map.png")
...
def robust_spawn_prop(env: Any, target_world: List[float]) -> bool:
    ...
    print(f"Attempting to spawn prop 'suspicious_target_0' at [{tx:.2f}, {ty:.2f}, {tz:.2f}]...")
...
def generate_trajectory_preview(
    ...
    try:
        ...
        plt.scatter(target_cell[1], target_cell[0], color="gold", marker="^",
                    s=250, edgecolors="black", linewidths=1.5, zorder=8, label="Suspicious Target (2,30)")
        ...
        plt.title(
            f"Phase 3B-min OpenWater Target Search: baseline_GP decision + HoloOcean execution\n"
...
def write_summary_report(
    ...
    header = f"""# Phase 3B-min: Single-USV Target Search Closed Loop Report
...
def main() -> None:
    print("=== Phase 3B-min: Single-USV target search closed loop ===")
    ...
    # 2. Map target_cell (2, 30) to world coordinates and verify roundtrip
    target_cell = (2, 30)
    ...
    print("Belief initialization sync completed for target at (2,30).")

    # 4. Target Manifest Output
    target_manifest = {
        "target_id": "suspicious_target_0",
    ...
    with holoocean.make(scenario_cfg=scenario_cfg) as env:
        ...
        for step in range(1, MAX_STEPS + 1):
            ...
            decision_record: Dict[str, Any] = {
                ...
                "clue_mode_note":                    "clue_acquisition_mode = ucb temporary for Phase 3B-min",
                # Position / path
    ...
    print("Phase 3B-min smoke complete!")
