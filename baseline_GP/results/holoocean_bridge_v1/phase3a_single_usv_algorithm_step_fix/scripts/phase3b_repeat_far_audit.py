"""Phase 3B-repeat-far target search audit script.

Validates the far-distance target search closed loop manifests.
Writes target_search_far_audit.json and target_search_far_audit_summary.md.
"""
...
DECISIONS_JSON    = os.path.join(PHASE_DIR, "manifests", "target_search_far_decisions.json")
TICK_CSV          = os.path.join(PHASE_DIR, "manifests", "target_search_far_holoocean_tick_trace.csv")
POLICY_TRACE_JSON = os.path.join(PHASE_DIR, "manifests", "target_search_far_policy_trace_rows.json")
TARGET_JSON       = os.path.join(PHASE_DIR, "manifests", "target_search_far_target_manifest.json")
FOUND_JSON        = os.path.join(PHASE_DIR, "manifests", "target_search_far_found_events.json")
SUMMARY_MD_IN     = os.path.join(PHASE_DIR, "reports",   "target_search_far_summary.md")
ROUTE_MAP_PNG     = os.path.join(PHASE_DIR, "visuals",   "target_search_far_route_map.png")

AUDIT_JSON_OUT    = os.path.join(PHASE_DIR, "manifests", "target_search_far_audit.json")
AUDIT_SUMMARY_OUT = os.path.join(PHASE_DIR, "reports",   "target_search_far_audit_summary.md")

SMOKE_SCRIPT_PATH = os.path.join(
    PHASE_DIR, "scripts", "openwater_single_usv_target_search_far.py"
)
BRIDGE_INIT_PATH  = os.path.join(BASE_DIR, "holoocean_bridge", "__init__.py")

MAX_STEPS = 250
RUN_LABEL = "target_search_far"
...
def main() -> None:
    print("=== Phase 3B-repeat-far: Single-USV Far-Distance Target Search Audit ===")

    checks: Dict[str, bool] = {}

    # Load decisions
    if not os.path.exists(DECISIONS_JSON):
        raise FileNotFoundError(f"Decisions manifest not found: {DECISIONS_JSON}. Run far smoke first.")
    ...
    checks["1_decisions_json_exists"] = _chk("1. target_search_far_decisions.json exists", os.path.exists(DECISIONS_JSON))

    # 2. Target manifest exists
    checks["2_target_manifest_exists"] = _chk("2. target_search_far_target_manifest.json exists", os.path.exists(TARGET_JSON))

    # 3. Found events exists
    checks["3_found_events_exists"] = _chk("3. target_search_far_found_events.json exists", os.path.exists(FOUND_JSON))

    # 4. Tick trace exists
    checks["4_tick_trace_exists"] = _chk("4. target_search_far_holoocean_tick_trace.csv exists", os.path.exists(TICK_CSV))

    # 5. Route map PNG exists
    checks["5_route_map_exists"] = _chk("5. target_search_far_route_map.png exists", os.path.exists(ROUTE_MAP_PNG))

    # 6. Target cell == [45, 60]
    target_cell = target_manifest.get("target_cell")
    checks["6_target_cell_is_correct"] = _chk("6. target_cell == [45, 60]", target_cell == [45, 60], f"actual={target_cell}")
    ...
    # 28. Far-specific: target_world matches [100.0, -25.0, 0.0] within tolerance
    tw = target_manifest.get("target_world")
    tw_ok = False
    if tw and len(tw) == 3:
        tw_ok = (abs(tw[0] - 100.0) < 0.1 and abs(tw[1] - (-25.0)) < 0.1 and abs(tw[2] - 0.0) < 0.1)
    checks["28_target_world_coordinate_correct"] = _chk("28. target_world is [100.0, -25.0, 0.0] within tolerance", tw_ok, f"actual={tw}")

    # 29. Far-specific: actual_steps <= 250
    checks["29_actual_steps_within_limit"] = _chk("29. actual_steps <= 250", actual_steps <= MAX_STEPS, f"actual={actual_steps}")

    # 30. Far-specific: terminated_reason is valid
    checks["30_terminated_reason_valid"] = _chk("30. terminated_reason is valid", terminated_reason in ["all_found", "max_steps_reached"])

    # 31. Far-specific: if found == false, found_step must be null
    f_step_null_ok = True
    ...
    # 32. Far-specific: if found == true, found_step must not be null
    ...
    # 33. Far-specific: if found == false, verify max_steps_reached and records final_cell, target_cell, distance
    no_found_manifest_ok = True
    if found_events.get("found") is False:
        no_found_manifest_ok = (
            found_events.get("reason") == "max_steps_reached"
            and "final_cell" in found_events
            and found_events.get("target_cell") == [45, 60]
    ...
    summary_lines = [
        f"# Phase 3B-repeat-far: Single-USV Target Search Audit Report",
        f"",
    ...
    summary_lines += [
        f"",
        f"---",
        f"*Generated by phase3b_repeat_far_audit.py – Phase 3B-repeat-far target search closed-loop verification.*",
    ]
    ...
    if all_passed:
        print(f"\n*** Phase 3B-repeat-far Audit: SUCCESS ***")
        print(f"All {total_checks} checks passed.")
    else:
        failed = [k for k, v in checks.items() if not v]
        print(f"\n*** Phase 3B-repeat-far Audit: FAILED ***")
