"""Audit for Phase 5D-1A local range-scaled SphereAgent blob validation."""
...
PROBE_SCRIPT = os.path.join(PHASE_DIR, "scripts", "sphere_blob_local_range_scaled_probe.py")
AUDIT_SCRIPT = os.path.join(PHASE_DIR, "scripts", "sphere_blob_local_range_scaled_audit.py")
...
BASE_COMPILE_LOG = os.path.join(PHASE_DIR, "logs", "phase5d1a_local_py_compile_base.txt")
HOLO_COMPILE_LOG = os.path.join(PHASE_DIR, "logs", "phase5d1a_local_py_compile_holo.txt")
AUDIT_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d1a_local_audit.json")
AUDIT_MD = os.path.join(PHASE_DIR, "reports", "phase5d1a_local_audit_summary.md")

EXPECTED_SCENE_COUNT = 53
EXPECTED_TARGET_ONLY_COUNT = 20
EXPECTED_TEAMMATE_ONLY_COUNT = 22
EXPECTED_TARGET_WITH_TEAMMATE_COUNT = 9
...
def _build_md(checks: Dict[str, bool], metrics: Dict[str, Any]) -> str:
    lines = [
        "# Phase 5D-1A Local Range-Scaled Blob Audit Summary",
        "",
...
def main() -> None:
    ...
    checks["recommended_rule_is_local_range_scaled"] = (
        recommended.get("rule_id") == "sphere_blob_local_range_scaled_any_hit"
        and recommended.get("rule_name") == "sphere_blob_local_range_scaled_any_hit"
        and recommended.get("uses_local_range_scaled_blob") is True
    ...
    checks["candidate_reference_present"] = (
        any(row.get("rule_name") == "sphere_blob_any_hit" for row in candidate_summaries)
        and any(row.get("rule_name") == "sphere_blob_local_range_scaled_any_hit" for row in candidate_summaries)
    ...
    checks["all_target_only_true_positive"] = (
        len(target_only_rows) == EXPECTED_TARGET_ONLY_COUNT
        and all(row.get("found") is True and row.get("outcome") == "true_positive" for row in target_only_rows)
    )
    checks["all_target_with_teammate_true_positive"] = (
        len(target_with_teammate_rows) == EXPECTED_TARGET_WITH_TEAMMATE_COUNT
        and all(row.get("found") is True and row.get("outcome") == "true_positive" for row in target_with_teammate_rows)
    )
    ...
    checks["local_blob_evidence_present_for_hits"] = all(
        row.get("local_range_scaled_blob_present") is True
        and row.get("local_blob_range_scaled_area") is not None
        and row.get("matched_beam_index") is not None
