"""Phase 5D-1A: local range-scaled SphereAgent blob validation.

This phase repairs and validates the heterogeneous static target rule from 5D-0/5D-1:
...
PHASE_NAME = "phase5d1a_sphere_blob_local_range_scaled_validation"
...
CAL_CONFIG_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d1a_local_calibration_config.json")
CAL_SCENE_RESULTS_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d1a_local_calibration_scene_results.json")
CAL_TICK_TRACE_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d1a_local_calibration_tick_trace.json")
CAL_TICK_TRACE_CSV = os.path.join(PHASE_DIR, "manifests", "phase5d1a_local_calibration_tick_trace.csv")
RECOMPUTED_EVIDENCE_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d1a_local_recomputed_rgb_evidence.json")
CANDIDATE_RULE_MATRIX_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d1a_local_candidate_rule_matrix.json")
CANDIDATE_RULE_MATRIX_CSV = os.path.join(PHASE_DIR, "manifests", "phase5d1a_local_candidate_rule_matrix.csv")
CANDIDATE_RULE_SUMMARY_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d1a_local_candidate_rule_summary.json")
RECOMMENDED_RULE_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d1a_local_recommended_candidate_rule.json")
CAL_SUMMARY_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d1a_local_calibration_summary.json")
CAL_GIT_STATUS_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d1a_local_calibration_git_status.json")
CAL_SUMMARY_MD = os.path.join(PHASE_DIR, "reports", "phase5d1a_local_calibration_summary.md")

VAL_CONFIG_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d1a_local_validation_config.json")
VAL_SCENE_RESULTS_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d1a_local_validation_scene_results.json")
VAL_DETECTION_EVENTS_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d1a_local_validation_detection_events.json")
VAL_RULE_MATRIX_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d1a_local_validation_rule_matrix.json")
VAL_RULE_MATRIX_CSV = os.path.join(PHASE_DIR, "manifests", "phase5d1a_local_validation_rule_matrix.csv")
VAL_TICK_TRACE_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d1a_local_validation_tick_trace.json")
VAL_TICK_TRACE_CSV = os.path.join(PHASE_DIR, "manifests", "phase5d1a_local_validation_tick_trace.csv")
VAL_SUMMARY_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d1a_local_validation_summary.json")
VAL_GIT_STATUS_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d1a_local_validation_git_status.json")
VAL_SUMMARY_MD = os.path.join(PHASE_DIR, "reports", "phase5d1a_local_validation_summary.md")
...
LOCAL_BLOB_MIN_AREA = 12
LOCAL_BLOB_MIN_FILL = 0.6
LOCAL_BLOB_MIN_RANGE_SCALED_AREA = 12000.0
LOCAL_BLOB_BEAM_YAW_TOLERANCE_DEG = 7.6
LOCAL_BLOB_CAMERA_HORIZONTAL_FOV_DEG = 106.0
...
TARGET_Z = 0.5
TEAMMATE_Z = 0.0
TARGET_DISTANCES_M = [20.0, 25.0, 30.0, 35.0]
TEAMMATE_SINGLE_DISTANCES_M = [20.0, 25.0, 30.0, 35.0]
...
FIXED_RECOMMENDED_RULE = {
    "rule_id": "sphere_blob_local_range_scaled_any_hit",
    "rule_name": "sphere_blob_local_range_scaled_any_hit",
    "rule_family": "sphere_blob_local_range_scaled",
    ...
    "uses_local_range_scaled_blob": True,
    "local_blob_min_area": LOCAL_BLOB_MIN_AREA,
    "local_blob_min_fill": LOCAL_BLOB_MIN_FILL,
    "local_blob_min_range_scaled_area": LOCAL_BLOB_MIN_RANGE_SCALED_AREA,
    "local_blob_beam_yaw_tolerance_deg": LOCAL_BLOB_BEAM_YAW_TOLERANCE_DEG,
    "source_phase": "phase5d1a_sphere_blob_local_range_scaled_validation",
}
...
def _build_scenes() -> List[Dict[str, Any]]:
    ...
    coexist_specs = [
        {
            "name": "coexist_sphere_center_20m_teammate_left_inner_20m",
            "target": _object_spec(TARGET_AGENT_TYPE, observer_world, 20.0, 0.0, TARGET_Z),
        ...
        {
            "name": "coexist_sphere_center_35m_teammate_right_inner_20m",
            "target": _object_spec(TARGET_AGENT_TYPE, observer_world, 35.0, 0.0, TARGET_Z),
            "teammates": [_object_spec(TEAMMATE_NEGATIVE_AGENT_TYPE, observer_world, 20.0, -15.0, TEAMMATE_Z)],
            "distance_m": 35.0,
            "angle_label": "target_center_teammate_right_inner",
            "angle_deg": 0.0,
        ...
        {
            "name": "coexist_sphere_left_inner_35m_teammate_right_outer_35m",
            "target": _object_spec(TARGET_AGENT_TYPE, observer_world, 35.0, 15.0, TARGET_Z),
            "teammates": [_object_spec(TEAMMATE_NEGATIVE_AGENT_TYPE, observer_world, 35.0, -25.0, TEAMMATE_Z)],
            "distance_m": 35.0,
            "angle_label": "target_left_inner_teammate_right_outer",
            "angle_deg": 15.0,
        },
...
def _phase_config(mode: str, pre_git_status: Optional[List[str]] = None, recommended_rule: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "mode": mode,
        "purpose": "Local range-scaled SphereAgent blob validation under target-only, teammate-only, and SurfaceVessel coexistence scenes",
        ...
        "found_rule": "sphere_blob_local_range_scaled_any_hit",
        ...
        "rgb_signature_detector": {
            ...
            "sphere_blob_detector": {
                ...
                "local_range_scaled_rule": {
                    "min_area": LOCAL_BLOB_MIN_AREA,
                    "min_range_scaled_area": LOCAL_BLOB_MIN_RANGE_SCALED_AREA,
                    "beam_yaw_tolerance_deg": LOCAL_BLOB_BEAM_YAW_TOLERANCE_DEG,
                    "camera_horizontal_fov_deg": LOCAL_BLOB_CAMERA_HORIZONTAL_FOV_DEG,
                },
            },
        },
...
def _blob_yaw_deg(blob: Optional[Dict[str, Any]]) -> Optional[float]:
    if not isinstance(blob, dict) or blob.get("cx") is None:
        return None
    return float((float(CAMERA_WIDTH) / 2.0 - float(blob["cx"])) * float(LOCAL_BLOB_CAMERA_HORIZONTAL_FOV_DEG) / float(CAMERA_WIDTH))
...
def _matched_blob_range(blob: Optional[Dict[str, Any]], raw_beams: Any) -> Dict[str, Any]:
    ...
    if yaw is None or raw_beams is None:
        return {"matched": False, "blob_yaw_deg": yaw, "matched_beam_index": None, "matched_beam_yaw_deg": None, "matched_range_m": None, "beam_yaw_error_deg": None}
    arr = np.asarray(raw_beams).astype(float).reshape(-1)
    candidates: List[Tuple[float, int, float, float]] = []
    for index, value in enumerate(arr.tolist()):
        if index >= len(FAN_YAW_DEGREES):
            continue
        range_m = float(value)
        if range_m <= 0.0:
            continue
        beam_yaw = float(FAN_YAW_DEGREES[index])
        error = abs(beam_yaw - float(yaw))
        if error <= float(LOCAL_BLOB_BEAM_YAW_TOLERANCE_DEG):
...
def _local_range_scaled_blob_signature(blobs: Any, raw_beams: Any) -> Dict[str, Any]:
    ...
    viable = [
        ...
        and int(item.get("local_blob_area") or 0) >= int(LOCAL_BLOB_MIN_AREA)
    ...
    present = bool(
        ...
        and area >= int(LOCAL_BLOB_MIN_AREA)
        and match.get("matched") is True
        and scaled_area is not None
        and scaled_area >= float(LOCAL_BLOB_MIN_RANGE_SCALED_AREA)
    )
    return {
        ...
        "local_blob_area": area,
        "local_blob_min_area": int(LOCAL_BLOB_MIN_AREA),
        "local_blob_min_fill": float(LOCAL_BLOB_MIN_FILL),
        "local_blob_range_scaled_area": scaled_area,
        "local_blob_min_range_scaled_area": float(LOCAL_BLOB_MIN_RANGE_SCALED_AREA),
...
def _sphere_blob_signature(rgb_value: Any, baseline_value: Any) -> Dict[str, Any]:
    ...
    for index in range(1, int(count)):
        ...
        if (
            ...
            if fill >= float(LOCAL_BLOB_MIN_FILL):
                local_candidates.append(blob)
            if fill >= float(SPHERE_BLOB_MIN_FILL):
                candidates.append(blob)
    candidates = sorted(candidates, key=lambda item: int(item.get("area", 0)), reverse=True)
    local_candidates = sorted(local_candidates, key=lambda item: int(item.get("area", 0)), reverse=True)
    ...
    return {
        ...
        "local_candidate_blob_count": len(local_candidates),
        "local_candidate_blobs": local_candidates[:8],
        "local_best_blob": local_candidates[0] if local_candidates else None,
        ...
        "params": {
            ...
            "max_aspect": SPHERE_BLOB_MAX_ASPECT,
            "min_fill": SPHERE_BLOB_MIN_FILL,
            "local_min_fill": LOCAL_BLOB_MIN_FILL,
            "min_cy": SPHERE_BLOB_MIN_CY,
            "min_area_ratio": SPHERE_BLOB_MIN_AREA_RATIO,
            "close_kernel_wh": [SPHERE_BLOB_CLOSE_KERNEL_W, SPHERE_BLOB_CLOSE_KERNEL_H],
        },
...
def _rule_configs() -> Iterable[Dict[str, Any]]:
    ...
    yield {
        ...
        "uses_rgb_signature": False,
        "uses_sphere_blob": True,
        "uses_local_range_scaled_blob": False,
        "local_blob_min_area": None,
        "local_blob_min_range_scaled_area": None,
        "uses_rangefinder": True,
    }
    yield {
        "rule_id": "sphere_blob_local_range_scaled_any_hit",
...
def _rule_found(rule: Dict[str, Any], evidence: Dict[str, Any]) -> bool:
    ...
    if str(rule.get("rule_name")) == "sphere_blob_local_range_scaled_any_hit":
        return bool(evidence.get("local_range_scaled_blob_present", False) and any_hit)
        ...
        for evidence in evidence_rows:
            ...
            row = {
                **rule,
                ...
                "sphere_blob_local_candidate_count": evidence.get("sphere_blob_local_candidate_count"),
                "sphere_blob_best": evidence.get("sphere_blob_best"),
                "sphere_blob_local_best": evidence.get("sphere_blob_local_best"),
                "sphere_blob_white_neutral_changed_pixels": evidence.get("sphere_blob_white_neutral_changed_pixels"),
                "local_range_scaled_blob_present": evidence.get("local_range_scaled_blob_present"),
                "local_range_scaled_best_blob": evidence.get("local_range_scaled_best_blob"),
                "local_blob_range_scaled_area": evidence.get("local_blob_range_scaled_area"),
                "local_blob_area": evidence.get("local_blob_area"),
                "blob_yaw_deg": evidence.get("blob_yaw_deg"),
                "matched_beam_index": evidence.get("matched_beam_index"),
                "matched_range_m": evidence.get("matched_range_m"),
...
def _fixed_recommended_rule(summary_rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    fixed["recommendation_reason"] = (
        "Phase 5D-1A validates a local range-scaled SphereAgent blob + fan-RangeFinder rule "
        "to repair SurfaceVessel coexistence false negatives while preserving teammate-only rejection."
    )
...
def _apply_recommended_rule(evidence_rows: List[Dict[str, Any]], recommended: Dict[str, Any], config: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    for evidence in evidence_rows:
        ...
        uses_local_blob = str(recommended.get("rule_name")) == "sphere_blob_local_range_scaled_any_hit"
        row = {
            "scene": evidence.get("scene"),
            "scenario_kind": evidence.get("scenario_kind"),
            ...
            "sphere_rgb_target_signature": bool(
                int(evidence.get("rgb_signature_overlap_count", 0)) >= int(recommended.get("target_overlap_min") or 1)
                and int(evidence.get("rgb_changed_pixels", 0)) >= int(recommended.get("rgb_changed_pixels_min") or 0)
            ) if (not uses_blob and not uses_local_blob) else None,
            "sphere_blob_target_signature": bool(evidence.get("sphere_blob_present", False)) if uses_blob else None,
            "local_range_scaled_blob_target_signature": bool(evidence.get("local_range_scaled_blob_present", False)) if uses_local_blob else None,
            "sphere_blob_present": evidence.get("sphere_blob_present"),
            "sphere_blob_candidate_count": evidence.get("sphere_blob_candidate_count"),
            "sphere_blob_local_candidate_count": evidence.get("sphere_blob_local_candidate_count"),
            "sphere_blob_best": evidence.get("sphere_blob_best"),
            ...
            "local_range_scaled_best_blob": evidence.get("local_range_scaled_best_blob"),
            "local_blob_range_scaled_area": evidence.get("local_blob_range_scaled_area"),
            "local_blob_min_range_scaled_area": evidence.get("local_blob_min_range_scaled_area"),
            "local_blob_area": evidence.get("local_blob_area"),
            "local_blob_min_area": evidence.get("local_blob_min_area"),
            "blob_yaw_deg": evidence.get("blob_yaw_deg"),
            "matched_beam_index": evidence.get("matched_beam_index"),
            "matched_beam_yaw_deg": evidence.get("matched_beam_yaw_deg"),
            "matched_range_m": evidence.get("matched_range_m"),
            "beam_yaw_error_deg": evidence.get("beam_yaw_error_deg"),
...
def run_calibrate() -> Dict[str, Any]:
    ...
    with open(CAL_SUMMARY_MD, "w", encoding="utf-8") as f:
        f.write(_build_summary_md(summary, "Phase 5D-1A Local Range-Scaled Blob Calibration Summary"))
...
def run_validate() -> Dict[str, Any]:
    ...
    with open(VAL_SUMMARY_MD, "w", encoding="utf-8") as f:
        f.write(_build_summary_md(summary, "Phase 5D-1A Local Range-Scaled Blob HoloOcean Validation Summary"))
    post_git_status = _run_git_status()
