"""Phase 5C-2: dual-USV static-target HoloOcean multistep smoke.

This phase keeps the Phase 5C-1 runtime update chain, but replaces the
runtime-only one-cell motion boundary with HoloOcean physical execution and
sensor projection back to the runtime grid.
"""
...
def _make_scenario_config(spec: Dict[str, Any], start_world: Dict[int, List[float]]) -> Dict[str, Any]:
    ...
    return {
        "name": "dual_usv_static_holoocean_multistep_smoke",
        "world": spec["world"],
...
def _build_summary_md(summary: Dict[str, Any]) -> str:
    return """# Phase 5C-2 HoloOcean Multistep Smoke Summary
...
## Notes
- This is a short HoloOcean smoke; target hit is not required.
...
def _generate_paths_png(nav_map_prior: np.ndarray, policy_trace: List[Dict[str, Any]], tick_trace: List[Dict[str, Any]]) -> None:
    try:
        ...
        plt.title("Phase 5C-2 HoloOcean Dual-USV Static Multistep Smoke")
        plt.xlabel("Column")
        plt.ylabel("Row")
        plt.xlim(-0.5, 80.5)
        plt.ylim(80.5, -0.5)
...
def main() -> None:
    print("=== Phase 5C-2: dual-USV static HoloOcean multistep smoke ===")
    result = run_smoke()
    summary = result["summary"]
    print(
        "Phase 5C-2 smoke finished: reason={0}, policy_steps={1}, ticks={2}, found={3}".format(
