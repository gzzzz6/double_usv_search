"""Phase 5C-4H fan-area distance boundary probe.

This phase tests the effective distance of the current RGB signature plus
multi-ray fan RangeFinder method across the fan area, not only the camera
centerline.

The primary found rule remains:
    found = rgb_has_target_signature and any_rangefinder_hit

Directional beam matching is recorded only as a diagnostic for a later phase.
"""
...
CONFIG_JSON = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "rgb_multiray_fan_area_distance_config.json"))
SCENE_RESULTS_JSON = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "rgb_multiray_fan_area_distance_scene_results.json"))
DETECTION_EVENTS_JSON = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "rgb_multiray_fan_area_distance_detection_events.json"))
TICK_TRACE_JSON = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "rgb_multiray_fan_area_distance_tick_trace.json"))
TICK_TRACE_CSV = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "rgb_multiray_fan_area_distance_tick_trace.csv"))
MATRIX_JSON = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "rgb_multiray_fan_area_distance_boundary_matrix.json"))
MATRIX_CSV = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "rgb_multiray_fan_area_distance_boundary_matrix.csv"))
SUMMARY_JSON = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "rgb_multiray_fan_area_distance_summary.json"))
GIT_STATUS_JSON = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "rgb_multiray_fan_area_distance_git_status.json"))
SUMMARY_MD = os.path.normpath(os.path.join(PHASE_DIR, "reports", "rgb_multiray_fan_area_distance_summary.md"))
OVERVIEW_PNG = os.path.normpath(os.path.join(PHASE_DIR, "reports", "rgb_multiray_fan_area_distance_overview.png"))
...
def _phase_config(pre_git_status: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "purpose": "fan-area distance boundary probe for current RGB signature plus fan RangeFinder adapter",
        "scope": "target distances 20/35/50m across left/center/right fan angles; distractor-only negatives at 20/50m",
        "distances_m": DISTANCES_M,
        "distractor_distances_m": DISTRACTOR_DISTANCES_M,
        ...
        "matched_rangefinder_beam_hit_is_diagnostic_only": True,
...
def _scenario_config(scene: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "name": "phase5c4h_{0}".format(scene["name"]),
        "world": "OpenWater",
        "package_name": "Ocean",
        "main_agent": "sv0",
        "agents": agents,
...
def _make_events(scene_results: Dict[str, Dict[str, Any]], config: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]], Dict[str, Any]]:
    ...
    direction_mismatch_scenes = [str(row["scene"]) for row in matrix if row.get("found") and not row.get("matched_rangefinder_beam_hit")]
    conclusion = {
        ...
        "direction_mismatch_scenes": direction_mismatch_scenes,
        "direction_mismatch_count": len(direction_mismatch_scenes),
...
def _build_summary(config: Dict[str, Any], events: List[Dict[str, Any]], matrix: List[Dict[str, Any]], conclusion: Dict[str, Any], started: float) -> Dict[str, Any]:
    return {
        ...
        "terminated_reason": "fan_area_distance_boundary_complete",
        "tested_distances_m": DISTANCES_M,
        "distractor_tested_distances_m": DISTRACTOR_DISTANCES_M,
        "fan_test_angles": config.get("fan_test_angles"),
        ...
        "direction_mismatch_scenes": conclusion.get("direction_mismatch_scenes"),
        "direction_mismatch_count": conclusion.get("direction_mismatch_count"),
        "rgb_signature_calibration_distance_m": config.get("rgb_signature_calibration_distance_m"),
        "rgb_signature_calibration_scene": config.get("rgb_signature_calibration_scene"),
        ...
        "boundary_conclusion": "fan reliable up to {0} m; per-angle maxima {1}; distractor false positives {2}".format(
            conclusion.get("max_fan_reliable_distance_m"),
            conclusion.get("max_effective_distance_by_angle_m"),
...
def _build_summary_md(summary: Dict[str, Any]) -> str:
    lines = [
        ...
        "- Tested Fan Angles: `{0}`".format(summary.get("fan_test_angles")),
        "- RGB Calibration Scene: `{0}`".format(summary.get("rgb_signature_calibration_scene")),
        "- Max Effective Distance By Angle: `{0}`".format(summary.get("max_effective_distance_by_angle_m")),
        ...
        "- Direction Mismatch Scenes: `{0}`".format(summary.get("direction_mismatch_scenes")),
    ...
    for row in summary.get("distance_matrix", []):
        ...
        lines.append(
            "| `{0}` | `{1}` | `{2}` | `{3}` | `{4}` | `{5}` | `{6}` | `{7}` | `{8}` |".format(
                ...
                row.get("matched_rangefinder_beam_hit"),
                row.get("found"),
                row.get("outcome"),
            )
        )
...
def _save_overview_visual(matrix: List[Dict[str, Any]]) -> bool:
    try:
        ...
        rows = [
            row
            for row in matrix
            if row.get("scenario_kind") in ("target", "distractor_only")
            and row.get("distance_m") is not None
            and row.get("rgb_artifact")
            and os.path.exists(str(row.get("rgb_artifact")))
        ]
        if not rows:
            return False
        thumb_w, thumb_h = 220, 165
        cols = min(5, len(rows))
        grid_rows = int(math.ceil(float(len(rows)) / float(cols)))
        pad = 12
        header_h = 58
        footer_h = 56
        out_w = cols * thumb_w + (cols + 1) * pad
        out_h = grid_rows * (header_h + thumb_h + footer_h + pad) + pad
        ...
        for idx, row in enumerate(rows):
            ...
            y = pad + line * (header_h + thumb_h + footer_h + pad)
            ...
            draw.text((x + 8, y + 12), "{0} {1}m".format(row.get("angle_label"), row.get("distance_m")), fill=(20, 28, 38), font=title_font)
            draw.text((x + 8, y + 36), "{0} {1}".format(row.get("scenario_kind"), row.get("outcome")), fill=color, font=small_font)
            ...
            draw.text((x + 8, y + header_h + thumb_h + 30), "beam={0} match={1}".format(row.get("rangefinder_hit_beam_indices"), row.get("matched_rangefinder_beam_hit")), fill=(82, 92, 105), font=small_font)
...
def run_capture() -> Dict[str, Any]:
    ...
    print(
        "Phase 5C-4H capture finished: fan_reliable={0}, false_positives={1}, direction_mismatches={2}".format(
            summary["fan_reliable_distances_m"],
            summary["distractor_false_positive_scenes"],
            summary["direction_mismatch_count"],
        )
    )
    return summary
