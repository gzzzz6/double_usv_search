"""E2E Closed-loop static target search smoke test – Phase 4C: 10m OpenWater.

Runs E2E search policy decisions with A* and GP in HoloOcean 10m grid using the twin-propeller fast controller.
"""
...
def robust_spawn_prop(env: Any, target_world: List[float]) -> bool:
    ...
    print(f"Attempting to spawn prop 'suspicious_target_res10_static_0' at [{tx:.2f}, {ty:.2f}, {tz:.2f}]...")
    ...
    try:
        env.spawn_prop(
            ...
            sim_physics=False,
            material="gold",
            tag="suspicious_target_res10_static_0",
        )
    ...
    try:
        env.spawn_prop(
            ...
            False,
            "gold",
            "suspicious_target_res10_static_0",
        )
...
def generate_route_map(
    ...
    try:
        ...
        # Target cell
        plt.scatter(target_cell[1], target_cell[0], color="red", marker="X", s=180, zorder=5, label="Static Target (20,45)")
        ...
        plt.title(
            f"Phase 4C: 10m Closed-loop Static Target Search Route Map\n"
...
def write_summary_report(
    ...
    header = f"""# Phase 4C: E2E 10m OpenWater Static Target Search Summary Report
...
- **Static Target**: Spawning visual prop at `(20, 45)` (World: `[50.0, 200.0, 2.0]`)
...
def main() -> None:
    print("=== Phase 4C: Launching 10m Closed-Loop Static Target Search Smoke ===")
    ...
    roundtrip_cell = tuple(int(v) for v in world_to_grid(target_world_3d, config=adapter_config))
    ...
    scenario_cfg = {
        "name": "static_target_search_res10_smoke",
        "world": spec["world"],
    ...
    with holoocean.make(scenario_cfg=scenario_cfg) as env:
        ...
        prop_spawn_success = robust_spawn_prop(env, target_world_3d)
        ...
        for step in range(1, MAX_STEPS + 1):
            ...
            tx, ty = float(next_world_3d[0]), float(next_world_3d[1])
    ...
    print(f"\n==========================================")
    print(f"10m Static Search Smoke Completed!")
    ...
    target_manifest = {
        ...
        "spawn_prop_name": "suspicious_target_res10_static_0"
    }
    ...
    print("Phase 4C E2E static target search smoke complete!")
