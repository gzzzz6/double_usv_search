"""Static and dynamic audit script for HoloOcean Phase 4C E2E static target search."""
...
PHASE_DIR = os.path.normpath(os.path.join(BASE_DIR, "results", "holoocean_bridge_v1", "phase4c_res10_static_target_search"))

DECISIONS_JSON      = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "static_target_search_res10_decisions.json"))
DECISIONS_CSV       = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "static_target_search_res10_decisions.csv"))
TICK_CSV            = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "static_target_search_res10_tick_trace.csv"))
POLICY_TRACE_JSON   = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "static_target_search_res10_policy_trace_rows.json"))
TARGET_JSON         = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "static_target_search_res10_target_manifest.json"))
FOUND_JSON          = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "static_target_search_res10_found_events.json"))
SUMMARY_MD          = os.path.normpath(os.path.join(PHASE_DIR, "reports",   "static_target_search_res10_summary.md"))
PREVIEW_PNG         = os.path.normpath(os.path.join(PHASE_DIR, "visuals",   "static_target_search_res10_route_map.png"))

SMOKE_SCRIPT        = os.path.normpath(os.path.join(PHASE_DIR, "scripts",   "static_target_search_res10_smoke.py"))

AUDIT_JSON          = os.path.normpath(os.path.join(PHASE_DIR, "manifests", "static_target_search_res10_audit.json"))
AUDIT_MD            = os.path.normpath(os.path.join(PHASE_DIR, "reports",   "static_target_search_res10_audit_summary.md"))
...
def run_audits() -> Dict[str, Any]:
    ...
    if checks["target_json_parse_ok"]:
        checks["target_cell_is_20_45"] = (tm.get("target_cell") == [20, 45])
        ...
        if map_spec_ok:
            try:
                ...
                g_pt = world_to_grid(w_pt, cfg, (81, 81))
        ...
        checks["target_cell_is_20_45"] = False
        checks["target_world_is_correct"] = False
    ...
    if checks["decisions_json_parse_ok"] and len(dec) > 0:
        planned_anchors_aligned = True
        planned_segments_aligned = True
        ...
        for d in dec:
            # 1. Planned segment anchor check
            rpb = d["robot_pos_before"]
            nc = d["next_cell"]
            fpc = d["final_projected_cell"]
            
            # segment_path[0] == robot_pos_before is checked in smoke, verify in decisions
            if not d.get("arrived", False):
                all_arrived = False
            if d.get("timeout", False):
                all_zero_timeouts = False
                
            # proj cell matches nc
            if fpc != nc:
        ...
        max_final_dist = 0.0
        if checks["trace_json_parse_ok"]:
            try:
                # Group by step and get last tick's distance
                import collections
    ...
    if checks["found_json_parse_ok"]:
        ...
        if is_found:
            ...
            # Compute min distance and count of steps in range dynamically
            min_dist_cells = 999.0
            steps_in_range = 0
            target_cell_coord = np.array([20, 45])
            for d in dec:
                fpc = d["final_projected_cell"]
                dist_c = np.linalg.norm(np.array(fpc) - target_cell_coord)
                if dist_c < min_dist_cells:
                    min_dist_cells = dist_c
                if dist_c <= 5.0:
                    steps_in_range += 1
                    
            checks["not_found_branch_valid"] = (
                tr == "max_steps_reached" and
                fs is None and
                (dec[-1]["found_count"] == 0 if dec else True) and
                (abs(dec[-1]["intensity_mass_after"] - 1.0) < 1e-4 if dec else True)
            )
            
            checks["final_sv_cell"] = [int(v) for v in dec[-1]["final_projected_cell"]] if dec else [23, 49]
            checks["target_cell"] = [20, 45]
            checks["final_distance_to_target_cells"] = float(fe.get("final_distance_to_target_cells", 5.0))
            checks["min_distance_to_target_cells"] = float(min_dist_cells)
            checks["steps_within_sensor_range_count"] = int(steps_in_range)
            
            print(f"[WARN] Target was NOT found in E2E search loop! Final cell: {checks['final_sv_cell']}, dist to target: {checks['final_distance_to_target_cells']}")
    else:
...
def main():
    print("=== Running Phase 4C static and dynamic audits ===")
    results = run_audits()
    ...
    summary_content = f"""# Phase 4C Static & Dynamic Search Audit Summary Report
...
| :--- | :--- | :--- | :---: |
| **decisions_json_exists** | `static_target_search_res10_decisions.json` exists | `{results['checks']['decisions_json_exists']}` | {'✅' if results['checks']['decisions_json_exists'] else '❌'} |
| **decisions_csv_exists** | `static_target_search_res10_decisions.csv` exists | `{results['checks']['decisions_csv_exists']}` | {'✅' if results['checks']['decisions_csv_exists'] else '❌'} |
| **tick_trace_exists** | `static_target_search_res10_tick_trace.csv` exists | `{results['checks']['tick_trace_csv_exists']}` | {'✅' if results['checks']['tick_trace_csv_exists'] else '❌'} |
| **policy_trace_exists** | `static_target_search_res10_policy_trace_rows.json` exists| `{results['checks']['policy_trace_json_exists']}` | {'✅' if results['checks']['policy_trace_json_exists'] else '❌'} |
| **target_manifest_exists**| `static_target_search_res10_target_manifest.json` exists | `{results['checks']['target_json_exists']}` | {'✅' if results['checks']['target_json_exists'] else '❌'} |
| **found_events_exists** | `static_target_search_res10_found_events.json` exists | `{results['checks']['found_json_exists']}` | {'✅' if results['checks']['found_json_exists'] else '❌'} |
| **summary_md_exists** | `static_target_search_res10_summary.md` summary exists | `{results['checks']['summary_md_exists']}` | {'✅' if results['checks']['summary_md_exists'] else '❌'} |
| **route_map_exists** | `static_target_search_res10_route_map.png` route map exists | `{results['checks']['route_map_png_exists']}` | {'✅' if results['checks']['route_map_png_exists'] else '❌'} |
...
| **target_cell_is_20_45**| Static target truth cell set at `(20, 45)` | `{results['checks'].get('target_cell_is_20_45')}` | {'✅' if results['checks'].get('target_cell_is_20_45') else '❌'} |
| **target_world_correct**| Target world coordinate maps to [50, 200] | `{results['checks'].get('target_world_is_correct')}` | {'✅' if results['checks'].get('target_world_is_correct') else '❌'} |
...
| **final_sv_cell** | Final USV grid cell coordinate | `{results['checks'].get('final_sv_cell')}` | ⚪ |
| **target_cell** | Synchronized static target grid coordinate | `{results['checks'].get('target_cell')}` | ⚪ |
| **final_distance_to_target_cells** | Final grid distance to target cells | `{results['checks'].get('final_distance_to_target_cells')}` | ⚪ |
| **min_distance_to_target_cells** | Minimum grid distance to target cells | `{results['checks'].get('min_distance_to_target_cells')}` | ⚪ |
| **steps_within_sensor_range_count** | Number of steps within 5.0 cell sensor range | `{results['checks'].get('steps_within_sensor_range_count')}` | ⚪ |
"""
    else:
        summary_content += f"""
