"""Smoke test script for running a waypoint sequence in HoloOcean OpenWater."""

import os
import sys
from types import ModuleType

# 1. Mock baseline_GP.core_map to prevent TypeError on older Python versions (like Python 3.8 in conda)
mock_core_map = ModuleType('baseline_GP.core_map')
mock_core_map.FREE = 0
mock_core_map.OCCUPIED = 1
mock_core_map.UNKNOWN = -1
sys.modules['baseline_GP.core_map'] = mock_core_map

# 2. Dynamically load coordinate_adapter by translating unsupported syntax
# to bypass non-subscriptable typing and pipe operators on older Python 3.8
adapter_path = './baseline_GP/holoocean_bridge/coordinate_adapter.py'
with open(adapter_path, 'r', encoding='utf-8') as f:
    adapter_source = f.read()

# Make it python 3.8 compatible
adapter_source = adapter_source.replace("from __future__ import annotations", "")
adapter_source = "from __future__ import annotations\nfrom typing import Union, Tuple, List, Dict, Sequence\n" + adapter_source
adapter_source = adapter_source.replace("CoordinateAdapterConfig | None", "Union[CoordinateAdapterConfig, None]")
adapter_source = adapter_source.replace("Sequence[int] | np.ndarray | None", "Union[Sequence[int], np.ndarray, None]")
adapter_source = adapter_source.replace("Sequence[int] | np.ndarray", "Union[Sequence[int], np.ndarray]")
adapter_source = adapter_source.replace("tuple[", "Tuple[")
adapter_source = adapter_source.replace("list[", "List[")
adapter_source = adapter_source.replace("dict[", "Dict[")

mock_adapter = ModuleType('baseline_GP.holoocean_bridge.coordinate_adapter')
mock_adapter.__dict__.update({
    '__file__': os.path.abspath(adapter_path),
    '__name__': 'baseline_GP.holoocean_bridge.coordinate_adapter',
    '__package__': 'baseline_GP.holoocean_bridge',
})
sys.modules['baseline_GP.holoocean_bridge.coordinate_adapter'] = mock_adapter
exec(compile(adapter_source, adapter_path, 'exec'), mock_adapter.__dict__)

# 3. Similarly load scene_map_adapter
scene_map_path = './baseline_GP/holoocean_bridge/scene_map_adapter.py'
with open(scene_map_path, 'r', encoding='utf-8') as f:
    scene_source = f.read()

scene_source = scene_source.replace("from __future__ import annotations", "")
scene_source = "from __future__ import annotations\nfrom typing import Union, Tuple, List, Dict\n" + scene_source
scene_source = scene_source.replace("tuple[", "Tuple[")
scene_source = scene_source.replace("list[", "List[")
scene_source = scene_source.replace("dict[", "Dict[")

mock_scene_map = ModuleType('baseline_GP.holoocean_bridge.scene_map_adapter')
mock_scene_map.__dict__.update({
    '__file__': os.path.abspath(scene_map_path),
    '__name__': 'baseline_GP.holoocean_bridge.scene_map_adapter',
    '__package__': 'baseline_GP.holoocean_bridge',
})
sys.modules['baseline_GP.holoocean_bridge.scene_map_adapter'] = mock_scene_map
exec(compile(scene_source, scene_map_path, 'exec'), mock_scene_map.__dict__)

# 4. Now perform imports normally - Python will use the compatible modules in sys.modules
import csv
import json
import time
...
PHASE_DIR = os.path.join(BASE_DIR, "results", "holoocean_bridge_v1", "phase2c_openwater_waypoint_smoke")
TRACE_CSV_PATH = os.path.join(PHASE_DIR, "manifests", "openwater_waypoint_trace.csv")
TRACE_JSON_PATH = os.path.join(PHASE_DIR, "manifests", "openwater_waypoint_trace.json")
SUMMARY_MD_PATH = os.path.join(PHASE_DIR, "reports", "openwater_waypoint_smoke_summary.md")
PREVIEW_PNG_PATH = os.path.join(PHASE_DIR, "visuals", "openwater_waypoint_trajectory_preview.png")
...
def generate_trajectory_preview(
    ...
    try:
        ...
        plt.title("HoloOcean SurfaceVessel Grid-derived Waypoint Simulation\n(openwater_open_v1, shape=81x81, cell_size=5m)", fontsize=12, pad=10)
...
def write_summary_report(
    ...
    header = f"""# HoloOcean Waypoint Smoke Test Report - Phase 2C
...
def main():
    print("=== Phase 2C: Executing HoloOcean OpenWater Waypoint Smoke ===")
    
    # 1. Load the Static Map from Phase 2B
    ...
    # 5. Launch HoloOcean and run sequence
    import holoocean
    ...
    print("Phase 2C Smoke Test run finished successfully!")
