"""Phase 5D-15: multicamera RGB/RF coverage without stop-scan.
...
CONFIG_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d15_config.json")
PRODUCTION_CONTRACT_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d15_production_adapter_contract.json")
CALIBRATION_RUN_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d15_live_calibration_run.json")
TARGET_RUN_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d15_live_target_run.json")
TEAMMATE_RUN_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d15_live_teammate_only_run.json")
TICK_TRACE_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d15_live_tick_trace.json")
TICK_TRACE_CSV = os.path.join(PHASE_DIR, "manifests", "phase5d15_live_tick_trace.csv")
POLICY_TRACE_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d15_live_policy_trace.json")
POLICY_TRACE_CSV = os.path.join(PHASE_DIR, "manifests", "phase5d15_live_policy_trace.csv")
EVENTS_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d15_live_per_agent_detection_events.json")
EVENTS_CSV = os.path.join(PHASE_DIR, "manifests", "phase5d15_live_per_agent_detection_events.csv")
FUSION_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d15_live_candidate_fusion_trace.json")
FUSION_CSV = os.path.join(PHASE_DIR, "manifests", "phase5d15_live_candidate_fusion_trace.csv")
ADAPTER_TRACE_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d15_adapter_observe_trace.json")
ADAPTER_TRACE_CSV = os.path.join(PHASE_DIR, "manifests", "phase5d15_adapter_observe_trace.csv")
MAINLINE_UPDATE_TRACE_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d15_mainline_update_found_trace.json")
MAINLINE_UPDATE_TRACE_CSV = os.path.join(PHASE_DIR, "manifests", "phase5d15_mainline_update_found_trace.csv")
...
RAW_FRAME_SYNC_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d15_raw_frame_sync_diagnostic.json")
RAW_FRAME_SYNC_CSV = os.path.join(PHASE_DIR, "manifests", "phase5d15_raw_frame_sync_diagnostic.csv")
SUMMARY_MD = os.path.join(PHASE_DIR, "reports", "phase5d15_summary.md")
BASE_COMPILE_LOG = os.path.join(PHASE_DIR, "logs", "phase5d15_py_compile_base.txt")
HOLO_COMPILE_LOG = os.path.join(PHASE_DIR, "logs", "phase5d15_py_compile_holo.txt")
...
P5D3 = _load_module(SOURCE_5D3_PROBE, "phase5d15_p5d3_detection_source")
...
def _output_paths(*, target_only_demo: bool) -> Dict[str, str]:
    ...
    prefix = "phase5d15_target_only_demo"
...
def _phase_config(pre_git_status: List[str]) -> Dict[str, Any]:
    return {
        ...
        "target_placement_source": "phase5d15_fixed_openwater_cell_on_baseline_planner_forward_fan_path",
...
def _scenario_config(
    *,
    ...
    return {
        "name": "phase5d15_{0}".format(run_kind),
    ...
    *,
    ...
    if display_live and run_kind == "target":
        live_viewer = LiveSearchViewer(
            ...
            map_shape=nav_map_prior.shape,
            title="Phase 5D-15 Live Dual-USV Search",
            fps=display_fps,
    ...
    adapter = ReusableMainlinePerceptionAdapter(adapter_id="phase5d15_{0}".format(run_kind))
    adapter.reset_episode("phase5d15_{0}".format(run_kind))
    launch_ok = False
    error = ""
...
def _raw_frame_case_row(
    *,
    ...
    base = os.path.join(
        PHASE_DIR,
        "visuals",
        "phase5d15_step{0:02d}_tick{1:03d}_{2}".format(step, tick, agent_name),
    )
    if save_visuals:
        visuals = {
...
def _build_raw_frame_sync_diagnostic(
    *,
    ...
    return {
        "phase_name": PHASE_NAME,
        "diagnostic_name": "phase5d15_raw_frame_baseline_sync_diagnostic",
...
def _run_compile_log(path: str) -> None:
    ...
    result = subprocess.run(
        [
            ...
            "-m",
            "py_compile",
            os.path.join(PHASE_DIR, "scripts", "multicamera_coverage_no_stop_scan_probe.py"),
        ],
        cwd=os.path.abspath("."),
        check=False,
        text=True,
    ...
    summary.update(
        {
            ...
            "demo_note": (
                ...
                "use the full default run plus phase5d15 audit for formal evidence."
            ),
        }
    )
    return summary
...
def _build_summary_md(summary: Dict[str, Any]) -> str:
    title = (
        "# Phase 5D-15 Target-Only Demo Summary"
        if summary.get("target_only_demo") is True
        else "# Phase 5D-15 Continuous Rolling RGB/RF Sync Summary"
    )
    lines = [
        title,
        "",
        ...
        "",
        (
            "This is a target-only visualization/demo run. It intentionally skips the teammate-only negative case, so it is not formal 5D-15 audit evidence."
            if summary.get("target_only_demo") is True
            else
            ...
            else "Phase 5D-15 exercises the real planner callback and continuous HoloOcean capture path without post-arrival stop-scan. If it does not close all_found, proceed to the 5D-15 sensor coverage fallback."
        ),
        "",
    ]
    return "\n".join(lines)
...
def run_live_planner_callback_search(
    *,
    ...
    print("Running 5D-15 live calibration/reference planner callback run")
    ...
    print("Running 5D-15 live target planner callback run")
    ...
    if target_only_demo:
        print("Skipping 5D-15 teammate-only run because --target-only-demo is enabled")
        teammate_run = _skipped_live_case(
            "teammate_only",
            "--target-only-demo skips the negative teammate-only audit segment",
        )
    else:
        print("Running 5D-15 live teammate-only planner callback run")
