"""Phase 5D-12: live planner callback continuous HoloOcean search.
...
CONFIG_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d12_config.json")
PRODUCTION_CONTRACT_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d12_production_adapter_contract.json")
CALIBRATION_RUN_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d12_live_calibration_run.json")
TARGET_RUN_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d12_live_target_run.json")
TEAMMATE_RUN_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d12_live_teammate_only_run.json")
TICK_TRACE_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d12_live_tick_trace.json")
TICK_TRACE_CSV = os.path.join(PHASE_DIR, "manifests", "phase5d12_live_tick_trace.csv")
POLICY_TRACE_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d12_live_policy_trace.json")
POLICY_TRACE_CSV = os.path.join(PHASE_DIR, "manifests", "phase5d12_live_policy_trace.csv")
EVENTS_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d12_live_per_agent_detection_events.json")
EVENTS_CSV = os.path.join(PHASE_DIR, "manifests", "phase5d12_live_per_agent_detection_events.csv")
FUSION_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d12_live_candidate_fusion_trace.json")
FUSION_CSV = os.path.join(PHASE_DIR, "manifests", "phase5d12_live_candidate_fusion_trace.csv")
ADAPTER_TRACE_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d12_adapter_observe_trace.json")
ADAPTER_TRACE_CSV = os.path.join(PHASE_DIR, "manifests", "phase5d12_adapter_observe_trace.csv")
MAINLINE_UPDATE_TRACE_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d12_mainline_update_found_trace.json")
MAINLINE_UPDATE_TRACE_CSV = os.path.join(PHASE_DIR, "manifests", "phase5d12_mainline_update_found_trace.csv")
SUMMARY_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d12_summary.json")
GIT_STATUS_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d12_git_status.json")
RAW_FRAME_SYNC_JSON = os.path.join(PHASE_DIR, "manifests", "phase5d12_raw_frame_sync_diagnostic.json")
RAW_FRAME_SYNC_CSV = os.path.join(PHASE_DIR, "manifests", "phase5d12_raw_frame_sync_diagnostic.csv")
SUMMARY_MD = os.path.join(PHASE_DIR, "reports", "phase5d12_summary.md")
BASE_COMPILE_LOG = os.path.join(PHASE_DIR, "logs", "phase5d12_py_compile_base.txt")
HOLO_COMPILE_LOG = os.path.join(PHASE_DIR, "logs", "phase5d12_py_compile_holo.txt")
...
P5D3 = _load_module(SOURCE_5D3_PROBE, "phase5d12_p5d3_detection_source")
...
def _phase_config(pre_git_status: List[str]) -> Dict[str, Any]:
    return {
        ...
        "target_placement_source": "phase5d12_fixed_openwater_cell_on_baseline_planner_forward_fan_path",
...
def _scenario_config(
    *,
    ...
    return {
        "name": "phase5d12_{0}".format(run_kind),
...
def _run_live_case(
    *,
    ...
    adapter = ReusableMainlinePerceptionAdapter(adapter_id="phase5d12_{0}".format(run_kind))
    adapter.reset_episode("phase5d12_{0}".format(run_kind))
    launch_ok = False
    error = ""
...
def _raw_frame_case_row(
    *,
    ...
    target_rgb = None if target_capture is None else target_capture.get("rgb")
    baseline_rgb = None if baseline_capture is None else baseline_capture.get("rgb")
    ...
    base = os.path.join(
        PHASE_DIR,
        "visuals",
        "phase5d12_step{0:02d}_tick{1:03d}_{2}".format(step, tick, agent_name),
    )
    visuals = {
...
def _build_raw_frame_sync_diagnostic(
    *,
    ...
    rows = [
        _raw_frame_case_row(event=event, target_run=target_run, calibration_run=calibration_run)
        for event in selected
    ]
...
def _run_compile_log(path: str) -> None:
    ...
    result = subprocess.run(
        [
            ...
            "-m",
            "py_compile",
            os.path.join(PHASE_DIR, "scripts", "live_raw_frame_baseline_sync_probe.py"),
        ],
        cwd=os.path.abspath("."),
        check=False,
        text=True,
...
def _build_summary_md(summary: Dict[str, Any]) -> str:
    lines = [
        "# Phase 5D-12 Live Raw Frame/Baseline Sync Summary",
        "",
        ...
        "",
        (
            "Phase 5D-12 unexpectedly closed the static SphereAgent live search chain. Treat this as requiring a separate full closure audit before claiming migration completion."
            if summary.get("phase_goal_completed") is True
            else "Phase 5D-12 is a raw-frame diagnostic: it exercises the real planner callback and live HoloOcean capture path, saves selected target/baseline RGB evidence, and does not by itself claim the HoloOcean real search all_found chain is closed."
        ),
        "",
    ]
    return "\n".join(lines)
...
def run_live_planner_callback_search() -> Dict[str, Any]:
    ...
    print("Running 5D-12 live calibration/reference planner callback run")
    ...
    print("Running 5D-12 live target planner callback run")
    ...
    print("Running 5D-12 live teammate-only planner callback run")
