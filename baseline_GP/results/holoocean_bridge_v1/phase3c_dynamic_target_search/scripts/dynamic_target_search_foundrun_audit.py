"""HoloOcean Dynamic Target Search Audit Script - foundrun

Verifies all output requirements and compliance constraints for Phase 3C-1-found.
"""
...
SMOKE_PY = os.path.join(SCRIPTS_DIR, "dynamic_target_search_foundrun.py")
AUDIT_PY = os.path.join(SCRIPTS_DIR, "dynamic_target_search_foundrun_audit.py")

DECISIONS_JSON  = os.path.join(MANIFESTS_DIR, "dynamic_target_search_foundrun_decisions.json")
DECISIONS_CSV   = os.path.join(MANIFESTS_DIR, "dynamic_target_search_foundrun_decisions.csv")
TICK_CSV        = os.path.join(MANIFESTS_DIR, "dynamic_target_search_foundrun_tick_trace.csv")
TARGET_JSON     = os.path.join(MANIFESTS_DIR, "dynamic_target_search_foundrun_target_trace.json")
TARGET_CSV      = os.path.join(MANIFESTS_DIR, "dynamic_target_search_foundrun_target_trace.csv")
FOUND_JSON      = os.path.join(MANIFESTS_DIR, "dynamic_target_search_foundrun_found_events.json")
SUMMARY_MD      = os.path.join(REPORTS_DIR,   "dynamic_target_search_foundrun_summary.md")
PREVIEW_PNG     = os.path.join(VISUALS_DIR,   "dynamic_target_search_foundrun_route_map.png")

AUDIT_JSON      = os.path.join(MANIFESTS_DIR, "dynamic_target_search_foundrun_audit.json")
AUDIT_SUMMARY_MD = os.path.join(REPORTS_DIR,   "dynamic_target_search_foundrun_audit_summary.md")
...
def main():
    print("=== Phase 3C-1-found: Starting Dynamic Target Search Audit ===")
    ...
    if decisions and found_events and target_trace:
        # Check expected max steps (must be <= 120 steps)
        actual_steps = len(decisions)
        audit_results["expected_max_steps_ok"] = (actual_steps <= 120)
        if not audit_results["expected_max_steps_ok"]:
            failures.append(f"Actual steps ({actual_steps}) exceed expected max steps (120)")
        ...
        audit_results["run_label_ok"] = (first_d.get("run_label") == "dynamic_target_search_foundrun")
        ...
        for k in ["target_proxy_agent_name_ok", "target_motion_source_ok", "target_motion_mode_for_policy_ok", "run_label_ok", "world_is_openwater"]:
            if not audit_results[k]:
                failures.append(f"Invalid target proxy agent parameters: {k} (value: {first_d.get(k.replace('_ok','')) if k != 'target_proxy_agent_name_ok' else target_trace[0].get('target_proxy_agent_name')})")
        ...
        if is_found:
            ...
            logical_ok = (
                ...
                fc_final == 1 and
                dist_found <= sensor_range
            )
            if not logical_ok:
                failures.append(f"Found events logical failure: found_step={found_step}, actual_steps={actual_steps}, fc_final={fc_final}, dist_found={dist_found}, sensor_range={sensor_range}")
        else:
            fc_final = decisions[-1].get("found_count_after", 0)
    ...
    summary_content = f"""# Dynamic Target Search Audit Report - Phase 3C-1-found
...
| Run Label OK | `{"PASSED" if audit_results["run_label_ok"] else "FAILED"}` | run_label is dynamic_target_search_foundrun |
...
| Max Steps Compliant (<= 120) | `{"PASSED" if audit_results["expected_max_steps_ok"] else "FAILED"}` | Steps within bounds |
"""
    if failures:
        summary_content += "\n### Failures List\n"
