"""E2E Closed-loop integration test – Phase 3D-1: Single-USV Dynamic Target Search (fastsv).

sv uses control_scheme=0 fast controller, target remains control_scheme=1 waypoint proxy.
...
DECISIONS_JSON  = os.path.join(PHASE_DIR, "manifests", "dynamic_target_search_fastsv_decisions.json")
DECISIONS_CSV   = os.path.join(PHASE_DIR, "manifests", "dynamic_target_search_fastsv_decisions.csv")
TICK_CSV        = os.path.join(PHASE_DIR, "manifests", "dynamic_target_search_fastsv_tick_trace.csv")
TARGET_JSON     = os.path.join(PHASE_DIR, "manifests", "dynamic_target_search_fastsv_target_trace.json")
TARGET_CSV      = os.path.join(PHASE_DIR, "manifests", "dynamic_target_search_fastsv_target_trace.csv")
FOUND_JSON      = os.path.join(PHASE_DIR, "manifests", "dynamic_target_search_fastsv_found_events.json")
SUMMARY_MD      = os.path.join(PHASE_DIR, "reports",   "dynamic_target_search_fastsv_summary.md")
PREVIEW_PNG     = os.path.join(PHASE_DIR, "visuals",   "dynamic_target_search_fastsv_route_map.png")
...
def generate_trajectory_preview(
    ...
    try:
        ...
        plt.title(
            f"Phase 3D-1 Dynamic Target Search: HoloOcean target agent + baseline_GP search\n"
...
def write_summary_report(
    ...
    header = f"""# Phase 3D-1: Single-USV Dynamic Target Search Closed Loop Report
...
def main() -> None:
    print("=== Phase 3D-1: Launching Single-USV Dynamic Target Search (fastsv) ===")
    ...
    scenario_cfg = {
        "name": "dynamic_target_search_fastsv",
        "world": spec["world"],
    ...
    found_events = {
        ...
        "run_label": "dynamic_target_search_fastsv",
        "phase_note": "Phase 3D-1 fast search-USV controller run; sv uses control_scheme=0 fast controller, target remains control_scheme=1 waypoint proxy; OpenWater, no sonar/camera."
    }

    # Fast controller parameters
    MAX_FORCE = 8000.0
    MIN_FORCE = 1500.0
    TURN_GAIN = 0.8
    ...
    with holoocean.make(scenario_cfg=scenario_cfg) as env:
        for step in range(1, MAX_STEPS + 1):
            ...
            for _ in range(TARGET_MOTION_TICKS_PER_STEP):
                env.act("sv", np.array([0.0, 0.0], dtype=np.float32))  # sv stationary under scheme=0
            ...
            while not arrived and not timeout:
                # Read SV position & Orientation from state_tick
                loc_sv = get_sensor_vector(state_tick, "sv", "LocationSensor")
                gps_sv = get_sensor_vector(state_tick, "sv", "GPSSensor")
                orient_sv = get_sensor_vector(state_tick, "sv", "OrientationSensor")
                ...
                # -----------------------------------------------------------
                # Waypoint Proportional Controller logic for SV
                # -----------------------------------------------------------
                ...
                if dist_sv < DIST_SLOW_RADIUS_M:
                    forward = MIN_FORCE + (MAX_FORCE - MIN_FORCE) * (dist_sv / DIST_SLOW_RADIUS_M)
                else:
                    forward = MAX_FORCE
                ...
                command_sv = np.array([left_thrust, right_thrust], dtype=np.float32)

                # Send commands to simulator
                env.act("sv", command_sv)
                env.act("target", np.array([target_wp_x, target_wp_y], dtype=np.float32))  # Continuous pursuit for target
                ...
                if dist_sv < SV_ARRIVAL_RADIUS_M and proj_cell == next_cell:
                    arrived = True
                ...
                tick_entry = {
                    ...
                    # Added for Phase 3D-1
                    "sv_command_0": float(left_thrust),
                    "sv_command_1": float(right_thrust),
                    "sv_heading_deg": float(heading_deg),
                    "sv_target_heading_deg": float(target_heading_deg),
                    "sv_heading_error_deg": float(heading_error_deg),
                    "sv_control_scheme": 0,
                    "target_control_scheme": 1,
                }
            ...
            decision_record = {
                ...
                # Phase 3D-1 added fields
                "sv_control_scheme":                 0,
                "target_control_scheme":             1,
                "sv_controller_type":                "fast_twin_propeller_controller",
                "sv_controller_config": {
                    "MAX_FORCE": MAX_FORCE,
                    "MIN_FORCE": MIN_FORCE,
                    "TURN_GAIN": TURN_GAIN,
                    "DIST_SLOW_RADIUS_M": DIST_SLOW_RADIUS_M,
                    "SV_ARRIVAL_RADIUS_M": SV_ARRIVAL_RADIUS_M,
                    "MAX_TICKS_PER_CELL": MAX_TICKS_PER_CELL,
                },
    ...
    print("Phase 3D-1 smoke complete!")
