...
DECISIONS_JSON  = os.path.join(PHASE_DIR, "manifests", "dynamic_target_search_decisions.json")
DECISIONS_CSV   = os.path.join(PHASE_DIR, "manifests", "dynamic_target_search_decisions.csv")
TICK_CSV        = os.path.join(PHASE_DIR, "manifests", "dynamic_target_search_tick_trace.csv")
TARGET_JSON     = os.path.join(PHASE_DIR, "manifests", "dynamic_target_search_target_trace.json")
TARGET_CSV      = os.path.join(PHASE_DIR, "manifests", "dynamic_target_search_target_trace.csv")
FOUND_JSON      = os.path.join(PHASE_DIR, "manifests", "dynamic_target_search_found_events.json")
SUMMARY_MD      = os.path.join(PHASE_DIR, "reports",   "dynamic_target_search_summary.md")
PREVIEW_PNG     = os.path.join(PHASE_DIR, "visuals",   "dynamic_target_search_route_map.png")
...
"""
    ...
    for i, d in enumerate(decisions):
        ...
        rows.append(
            ...
            f"{d['distance_to_next_cell_m']:.2f} | {d.get('found_count',0)} | "
...
def main() -> None:
    print("=== Phase 3C-1: Launching Single-USV Dynamic Target Search ===")
    ...
    scenario_cfg = {
        "name": "dynamic_target_search_smoke",
        "world": spec["world"],
    ...
    found_events = {
        ...
        "run_label": "dynamic_target_search_mini",
        "phase_note": "Phase 3C-1-mini short-run dynamic target synchronization smoke; not final long-horizon dynamic experiment."
    }
