"""HoloOcean Dynamic Target Search Audit Script - catchable

Verifies all output requirements and compliance constraints for Phase 3C-1-catchable.
"""
...
SMOKE_PY = os.path.join(SCRIPTS_DIR, "dynamic_target_search_catchable.py")
AUDIT_PY = os.path.join(SCRIPTS_DIR, "dynamic_target_search_catchable_audit.py")

DECISIONS_JSON  = os.path.join(MANIFESTS_DIR, "dynamic_target_search_catchable_decisions.json")
DECISIONS_CSV   = os.path.join(MANIFESTS_DIR, "dynamic_target_search_catchable_decisions.csv")
TICK_CSV        = os.path.join(MANIFESTS_DIR, "dynamic_target_search_catchable_tick_trace.csv")
TARGET_JSON     = os.path.join(MANIFESTS_DIR, "dynamic_target_search_catchable_target_trace.json")
TARGET_CSV      = os.path.join(MANIFESTS_DIR, "dynamic_target_search_catchable_target_trace.csv")
FOUND_JSON      = os.path.join(MANIFESTS_DIR, "dynamic_target_search_catchable_found_events.json")
SUMMARY_MD      = os.path.join(REPORTS_DIR,   "dynamic_target_search_catchable_summary.md")
PREVIEW_PNG     = os.path.join(VISUALS_DIR,   "dynamic_target_search_catchable_route_map.png")

AUDIT_JSON      = os.path.join(MANIFESTS_DIR, "dynamic_target_search_catchable_audit.json")
AUDIT_SUMMARY_MD = os.path.join(REPORTS_DIR,   "dynamic_target_search_catchable_audit_summary.md")
...
def main():
    print("=== Phase 3C-1-catchable: Starting Dynamic Target Search Audit ===")
    ...
    if decisions and found_events and target_trace:
        ...
        audit_results["run_label_ok"] = (first_d.get("run_label") == "dynamic_target_search_catchable")
    ...
    summary_content = f"""# Dynamic Target Search Audit Report - Phase 3C-1-catchable
...
"""
    if not is_found:
        summary_content += "- **Note**: `catchable run did not trigger probabilistic detection`\n"
...
| Run Label OK | `{"PASSED" if audit_results["run_label_ok"] else "FAILED"}` | run_label is dynamic_target_search_catchable |
...
| Catchable Schedule Compliance | `{"PASSED" if audit_results["catchable_schedule_ok"] else "FAILED"}` | Contains (25,30) staying after step 46 |
"""
    if failures:
        summary_content += "\n### Failures List\n"
