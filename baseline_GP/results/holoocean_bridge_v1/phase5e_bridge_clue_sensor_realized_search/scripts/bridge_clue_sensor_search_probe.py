"""Phase 5E: multicamera RGB/RF coverage without stop-scan.
...
CONFIG_JSON = os.path.join(PHASE_DIR, "manifests", "phase5e_config.json")
PRODUCTION_CONTRACT_JSON = os.path.join(PHASE_DIR, "manifests", "phase5e_production_adapter_contract.json")
CALIBRATION_RUN_JSON = os.path.join(PHASE_DIR, "manifests", "phase5e_live_calibration_run.json")
TARGET_RUN_JSON = os.path.join(PHASE_DIR, "manifests", "phase5e_live_target_run.json")
TEAMMATE_RUN_JSON = os.path.join(PHASE_DIR, "manifests", "phase5e_live_teammate_only_run.json")
TICK_TRACE_JSON = os.path.join(PHASE_DIR, "manifests", "phase5e_live_tick_trace.json")
TICK_TRACE_CSV = os.path.join(PHASE_DIR, "manifests", "phase5e_live_tick_trace.csv")
POLICY_TRACE_JSON = os.path.join(PHASE_DIR, "manifests", "phase5e_live_policy_trace.json")
POLICY_TRACE_CSV = os.path.join(PHASE_DIR, "manifests", "phase5e_live_policy_trace.csv")
EVENTS_JSON = os.path.join(PHASE_DIR, "manifests", "phase5e_live_per_agent_detection_events.json")
EVENTS_CSV = os.path.join(PHASE_DIR, "manifests", "phase5e_live_per_agent_detection_events.csv")
FUSION_JSON = os.path.join(PHASE_DIR, "manifests", "phase5e_live_candidate_fusion_trace.json")
FUSION_CSV = os.path.join(PHASE_DIR, "manifests", "phase5e_live_candidate_fusion_trace.csv")
ADAPTER_TRACE_JSON = os.path.join(PHASE_DIR, "manifests", "phase5e_adapter_observe_trace.json")
ADAPTER_TRACE_CSV = os.path.join(PHASE_DIR, "manifests", "phase5e_adapter_observe_trace.csv")
MAINLINE_UPDATE_TRACE_JSON = os.path.join(PHASE_DIR, "manifests", "phase5e_mainline_update_found_trace.json")
MAINLINE_UPDATE_TRACE_CSV = os.path.join(PHASE_DIR, "manifests", "phase5e_mainline_update_found_trace.csv")
CLUE_TRACE_JSON = os.path.join(PHASE_DIR, "manifests", "phase5e_bridge_clue_observation_trace.json")
CLUE_TRACE_CSV = os.path.join(PHASE_DIR, "manifests", "phase5e_bridge_clue_observation_trace.csv")
...
RAW_FRAME_SYNC_JSON = os.path.join(PHASE_DIR, "manifests", "phase5e_raw_frame_sync_diagnostic.json")
RAW_FRAME_SYNC_CSV = os.path.join(PHASE_DIR, "manifests", "phase5e_raw_frame_sync_diagnostic.csv")
SUMMARY_MD = os.path.join(PHASE_DIR, "reports", "phase5e_summary.md")
BASE_COMPILE_LOG = os.path.join(PHASE_DIR, "logs", "phase5e_py_compile_base.txt")
HOLO_COMPILE_LOG = os.path.join(PHASE_DIR, "logs", "phase5e_py_compile_holo.txt")
...
CAMERA_CHANNELS = [
    {"id": "front", "camera_name": "FrontRGBCamera", "yaw_deg": 0.0},
    {"id": "left", "camera_name": "LeftRGBCamera", "yaw_deg": 90.0},
    {"id": "right", "camera_name": "RightRGBCamera", "yaw_deg": -90.0},
    {"id": "rear", "camera_name": "RearRGBCamera", "yaw_deg": 180.0},
]
DEFAULT_CAMERA_CHANNEL_ID = "front"
BRIDGE_FAN_LOCAL_YAW_DEGREES = [float(v) for v in range(30, -31, -1)]
...
RGB_SYNC_STRATEGY = "multicamera_continuous_rolling_detector_scored_nearest_pair"
DEFAULT_DISPLAY_FPS = 20.0
DEFAULT_DISPLAY_EVERY_TICKS = 4
BRIDGE_CLUE_SENSOR_ID = "phase5e_passive_anomaly_clue_sensor"
BRIDGE_CLUE_SENSOR_ROLE = "local_passive_anomaly_observation_for_gp_only"
BRIDGE_CLUE_SENSOR_FOOTPRINT = "local_grid_disk_from_holoocean_location_sensor_pose"
BRIDGE_CLUE_SOURCE_MODEL = "scene_registered_static_passive_anomaly_source_not_detection_truth"
BRIDGE_CLUE_SAMPLES_PER_USV_STEP = 6
BRIDGE_CLUE_SIGMA_M = 35.0
BRIDGE_CLUE_AMPLITUDE = 2.0
BRIDGE_CLUE_BACKGROUND = 0.02
BRIDGE_CLUE_NOISE_STD = 0.03
BRIDGE_CLUE_MAX_OBS = 256
...
P5D3 = _load_module(SOURCE_5D3_PROBE, "phase5e_p5d3_detection_source")
...
def _output_paths(*, target_only_demo: bool) -> Dict[str, str]:
    if not target_only_demo:
        return {
            "config_json": CONFIG_JSON,
            "production_contract_json": PRODUCTION_CONTRACT_JSON,
            "calibration_run_json": CALIBRATION_RUN_JSON,
            "target_run_json": TARGET_RUN_JSON,
            "teammate_run_json": TEAMMATE_RUN_JSON,
            "tick_trace_json": TICK_TRACE_JSON,
            "tick_trace_csv": TICK_TRACE_CSV,
            "policy_trace_json": POLICY_TRACE_JSON,
            "policy_trace_csv": POLICY_TRACE_CSV,
            "events_json": EVENTS_JSON,
            "events_csv": EVENTS_CSV,
            "fusion_json": FUSION_JSON,
            "fusion_csv": FUSION_CSV,
            "adapter_trace_json": ADAPTER_TRACE_JSON,
            "adapter_trace_csv": ADAPTER_TRACE_CSV,
            "mainline_update_trace_json": MAINLINE_UPDATE_TRACE_JSON,
            "mainline_update_trace_csv": MAINLINE_UPDATE_TRACE_CSV,
            "clue_trace_json": CLUE_TRACE_JSON,
            "clue_trace_csv": CLUE_TRACE_CSV,
            "raw_frame_sync_json": RAW_FRAME_SYNC_JSON,
            "raw_frame_sync_csv": RAW_FRAME_SYNC_CSV,
            "summary_json": SUMMARY_JSON,
            "git_status_json": GIT_STATUS_JSON,
            "summary_md": SUMMARY_MD,
        }
    prefix = "phase5e_target_only_demo"
    manifests = os.path.join(PHASE_DIR, "manifests")
    reports = os.path.join(PHASE_DIR, "reports")
    return {
        "config_json": os.path.join(manifests, "{0}_config.json".format(prefix)),
        "production_contract_json": os.path.join(manifests, "{0}_production_adapter_contract.json".format(prefix)),
        "calibration_run_json": os.path.join(manifests, "{0}_live_calibration_run.json".format(prefix)),
        "target_run_json": os.path.join(manifests, "{0}_live_target_run.json".format(prefix)),
        "teammate_run_json": os.path.join(manifests, "{0}_live_teammate_only_skipped_run.json".format(prefix)),
        "tick_trace_json": os.path.join(manifests, "{0}_live_tick_trace.json".format(prefix)),
        "tick_trace_csv": os.path.join(manifests, "{0}_live_tick_trace.csv".format(prefix)),
        "policy_trace_json": os.path.join(manifests, "{0}_live_policy_trace.json".format(prefix)),
        "policy_trace_csv": os.path.join(manifests, "{0}_live_policy_trace.csv".format(prefix)),
        "events_json": os.path.join(manifests, "{0}_live_per_agent_detection_events.json".format(prefix)),
        "events_csv": os.path.join(manifests, "{0}_live_per_agent_detection_events.csv".format(prefix)),
        "fusion_json": os.path.join(manifests, "{0}_live_candidate_fusion_trace.json".format(prefix)),
        "fusion_csv": os.path.join(manifests, "{0}_live_candidate_fusion_trace.csv".format(prefix)),
        "adapter_trace_json": os.path.join(manifests, "{0}_adapter_observe_trace.json".format(prefix)),
        "adapter_trace_csv": os.path.join(manifests, "{0}_adapter_observe_trace.csv".format(prefix)),
        "mainline_update_trace_json": os.path.join(manifests, "{0}_mainline_update_found_trace.json".format(prefix)),
        "mainline_update_trace_csv": os.path.join(manifests, "{0}_mainline_update_found_trace.csv".format(prefix)),
        "clue_trace_json": os.path.join(manifests, "{0}_bridge_clue_observation_trace.json".format(prefix)),
        "clue_trace_csv": os.path.join(manifests, "{0}_bridge_clue_observation_trace.csv".format(prefix)),
        "raw_frame_sync_json": os.path.join(manifests, "{0}_raw_frame_sync_diagnostic.json".format(prefix)),
        "raw_frame_sync_csv": os.path.join(manifests, "{0}_raw_frame_sync_diagnostic.csv".format(prefix)),
        "summary_json": os.path.join(manifests, "{0}_summary.json".format(prefix)),
        "git_status_json": os.path.join(manifests, "{0}_git_status.json".format(prefix)),
        "summary_md": os.path.join(reports, "{0}_summary.md".format(prefix)),
    }
...
class LiveSearchViewer:
    ...
    def render(self, force_wait_ms: Optional[int] = None) -> None:
        ...
        try:
            ...
            cv2.putText(
                img,
                "found_mask={0}  RF_hit={1}  detected={2}  Q: exit simulation".format(
            ...
            if key in (ord("q"), ord("Q")):
                ...
                print("Q pressed: exiting HoloOcean simulation process.")
                raise SystemExit(0)
        except Exception as exc:
...
def _phase_config(pre_git_status: List[str]) -> Dict[str, Any]:
    return {
        ...
        "phase_goal": "Run baseline_GP mainline planner callback with HoloOcean bridge-local clue observations driving GP input, RGB/RF as the only found_mask trigger, continuous movement/capture, and no post-arrival stop-scan.",
        "phase_stage_plan": ["5E-0", "5E-1", "5E-2", "5E-3", "5E-4"],
        "phase_stage_scope": {
            "5E-0": "independent_phase_dir_and_baseline_core_freeze",
            "5E-1": "bridge_local_passive_anomaly_clue_sensor_defined",
            "5E-2": "gp_clue_observations_from_holoocean_bridge_pose_not_baseline_target_induced_field",
            "5E-3": "rgb_rf_only_found_mask_trigger_preserved",
            "5E-4": "audit_and_stage_report_generated",
        },
        ...
        "bridge_clue_sensor_enabled": True,
        "bridge_clue_sensor_id": BRIDGE_CLUE_SENSOR_ID,
        "bridge_clue_sensor_role": BRIDGE_CLUE_SENSOR_ROLE,
        "bridge_clue_sensor_footprint": BRIDGE_CLUE_SENSOR_FOOTPRINT,
        "bridge_clue_source_model": BRIDGE_CLUE_SOURCE_MODEL,
        "bridge_clue_samples_per_usv_step": BRIDGE_CLUE_SAMPLES_PER_USV_STEP,
        "bridge_clue_sigma_m": BRIDGE_CLUE_SIGMA_M,
        "bridge_clue_amplitude": BRIDGE_CLUE_AMPLITUDE,
        "bridge_clue_background": BRIDGE_CLUE_BACKGROUND,
        "bridge_clue_noise_std": BRIDGE_CLUE_NOISE_STD,
        "bridge_clue_max_observations": BRIDGE_CLUE_MAX_OBS,
        "baseline_target_induced_clue_used_for_gp": False,
        "baseline_target_induced_clue_sampling_disabled_in_phase5e": True,
        "bridge_clue_observation_used_for_gp": True,
        "clue_directly_updates_found_mask": False,
        "rgb_rf_detection_updates_found_mask": True,
        "gp_clue_source": "holoocean_bridge_local_clue_sensor_observations",
        ...
        "target_placement_source": "phase5e_fixed_openwater_cell_on_baseline_planner_forward_fan_path",
        ...
        "bridge_fan_rangefinder_sensor_count": len(CAMERA_CHANNELS) * len(BRIDGE_FAN_LOCAL_YAW_DEGREES),
        "bridge_fan_local_yaw_degrees": list(BRIDGE_FAN_LOCAL_YAW_DEGREES),
        "bridge_fan_yaw_degrees": list(BRIDGE_FAN_YAW_DEGREES),
        "bridge_fan_geometry_role": "multicamera_dense_bridge_rangefinder_sampling_for_live_planner_motion_not_detection_rule_change",
...
def _initialize_state(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if len(fixed_cell) == 2:
        ...
        state["clue_field_fn"] = None
        state["clue_true_map"] = None
        runtime_2usv._refresh_team_gp_state(state, optimize_hyperparams=False)
        runtime_2usv._update_team_search_info_state(state, state["intensity_map"])
    state["bridge_clue_rows"] = []
    state["bridge_clue_rng"] = np.random.default_rng(int(config["episode_seed"]) + 5000)
    state["bridge_clue_gp_update_count"] = 0
    state["baseline_target_induced_clue_used_for_gp"] = False
    return state
...
def _camera_channel(channel_id: str) -> Dict[str, Any]:
...
def _fan_rangefinder_prefix(channel_id: str) -> str:
    return "{0}_{1}".format(P5D3.FAN_RANGEFINDER_PREFIX, channel_id)


def _world_xy_from_cell(cell: Tuple[int, int], adapter_config: Any) -> List[float]:
    world = grid_to_world((int(cell[0]), int(cell[1])), config=adapter_config)
    return [float(world[0]), float(world[1])]


def _bridge_clue_value(
    *,
    sample_world_xy: List[float],
    source_world_xy: List[float],
    rng: np.random.Generator,
) -> Tuple[float, float, float]:
    dx = float(sample_world_xy[0]) - float(source_world_xy[0])
    dy = float(sample_world_xy[1]) - float(source_world_xy[1])
    distance_m = float(math.hypot(dx, dy))
    signal = float(BRIDGE_CLUE_AMPLITUDE) * math.exp(
        -(distance_m * distance_m) / (2.0 * float(BRIDGE_CLUE_SIGMA_M) * float(BRIDGE_CLUE_SIGMA_M))
    )
    noise = float(rng.normal(0.0, float(BRIDGE_CLUE_NOISE_STD)))
    value = max(0.0, float(BRIDGE_CLUE_BACKGROUND) + signal + noise)
    return float(value), float(signal), float(distance_m)


def _bridge_clue_candidate_cells(
    *,
    ...
    cells = []
    for cell in runtime_2usv.sensor_cells(center_cell, nav_map_prior.shape, int(sensor_range_cells)):
        if nav_map_prior[cell] == runtime_2usv.FREE:
            cells.append((int(cell[0]), int(cell[1])))
    return cells


def _apply_bridge_clue_observations(
    *,
    state: Dict[str, Any],
    adapter_config: Any,
    policy_step: int,
    run_kind: str,
    observer_world_by_usv: Dict[int, List[float]],
    source_world_location: List[float],
    observation_phase: str,
) -> List[Dict[str, Any]]:
    rng = state["bridge_clue_rng"]
    rows: List[Dict[str, Any]] = []
    X_batches: List[List[float]] = []
    y_values: List[float] = []
    source_xy = [float(source_world_location[0]), float(source_world_location[1])]
    for usv_id, observer_world in observer_world_by_usv.items():
        observer_cell = world_to_grid(observer_world, config=adapter_config)
        candidates = _bridge_clue_candidate_cells(
            center_cell=(int(observer_cell[0]), int(observer_cell[1])),
            nav_map_prior=state["nav_map_prior"],
            sensor_range_cells=int(state["sensor_range_cells"]),
        )
        if not candidates:
            continue
        sample_count = min(int(BRIDGE_CLUE_SAMPLES_PER_USV_STEP), len(candidates))
        indices = rng.choice(len(candidates), size=sample_count, replace=False)
        for local_index, candidate_index in enumerate(np.asarray(indices).astype(int).tolist()):
            cell = candidates[int(candidate_index)]
            world_xy = _world_xy_from_cell(cell, adapter_config)
            value, signal, distance_m = _bridge_clue_value(
                sample_world_xy=world_xy,
                source_world_xy=source_xy,
                rng=rng,
            )
            X_batches.append(world_xy)
            y_values.append(value)
            row = {
                "phase_name": PHASE_NAME,
                "run_kind": run_kind,
                "policy_step": int(policy_step),
                "observation_phase": str(observation_phase),
                "sensor_id": BRIDGE_CLUE_SENSOR_ID,
                "sensor_role": BRIDGE_CLUE_SENSOR_ROLE,
                "sensor_footprint": BRIDGE_CLUE_SENSOR_FOOTPRINT,
                "source_model": BRIDGE_CLUE_SOURCE_MODEL,
                "reporter_usv_id": "sv{0}".format(int(usv_id)),
                "observer_world_location": [float(v) for v in observer_world],
                "observer_grid_cell": [int(observer_cell[0]), int(observer_cell[1])],
                "sample_index": int(local_index),
                "sample_grid_cell": [int(cell[0]), int(cell[1])],
                "sample_world_xy": list(world_xy),
                "clue_value": float(value),
                "noise_std": float(BRIDGE_CLUE_NOISE_STD),
                "signal_without_noise": float(signal),
                "distance_to_registered_source_m": float(distance_m),
                "registered_source_world_location_for_audit": [float(v) for v in source_world_location],
                "registered_source_not_exposed_to_planner": True,
                "used_for_gp_observation": True,
                "used_for_detection": False,
                "directly_updates_found_mask": False,
                "detected_mask_from_clue": False,
    ...
    if X_batches:
        state["gp_field"].add_observations(
            np.asarray(X_batches, dtype=float),
            np.asarray(y_values, dtype=float),
            t=float(policy_step),
        )
        if state["gp_max_points"] is not None:
            state["gp_field"].prune_observations(
                current_time=float(policy_step),
                max_points=min(int(state["gp_max_points"]), int(BRIDGE_CLUE_MAX_OBS)),
            )
        state["current_step"] = int(policy_step)
        optimize_now = bool(
            state.get("gp_optimize_hyperparams", False)
            and int(state.get("gp_fit_every", 0) or 0) > 0
            and int(policy_step) % int(state.get("gp_fit_every", 1)) == 0
        )
        runtime_2usv._refresh_team_gp_state(state, optimize_hyperparams=optimize_now)
        runtime_2usv._update_team_search_info_state(state, state["intensity_map"])
        state["bridge_clue_gp_update_count"] = int(state.get("bridge_clue_gp_update_count", 0)) + 1
    state.setdefault("bridge_clue_rows", []).extend(rows)
    return rows


def _fan_rangefinder_names(channel_id: str = DEFAULT_CAMERA_CHANNEL_ID) -> List[str]:
    prefix = _fan_rangefinder_prefix(channel_id)
    return [
        "{0}_{1:02d}".format(prefix, index)
        for index in range(len(BRIDGE_FAN_LOCAL_YAW_DEGREES))
    ]


def _camera_sensor(channel: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "sensor_type": "RGBCamera",
        "sensor_name": str(channel["camera_name"]),
        "location": P5D3.P5D1A.CAMERA_LOCATION,
        "rotation": [0.0, 0.0, float(channel["yaw_deg"])],
        "Hz": P5D3.P5D1A.CAMERA_HZ,
        "configuration": {
            "CaptureWidth": P5D3.P5D1A.CAMERA_WIDTH,
            "CaptureHeight": P5D3.P5D1A.CAMERA_HEIGHT,
        },
    }
...
def _surface_vessel_sensors() -> List[Dict[str, Any]]:
    ...
    ] + [_camera_sensor(channel) for channel in CAMERA_CHANNELS] + _bridge_fan_rangefinder_sensors()


def _bridge_fan_rangefinder_sensors() -> List[Dict[str, Any]]:
    sensors: List[Dict[str, Any]] = []
    for channel in CAMERA_CHANNELS:
        channel_yaw = float(channel["yaw_deg"])
        prefix = _fan_rangefinder_prefix(str(channel["id"]))
        for index, local_yaw_deg in enumerate(BRIDGE_FAN_LOCAL_YAW_DEGREES):
            sensors.append(
                {
                    "sensor_type": "RangeFinderSensor",
                    "sensor_name": "{0}_{1:02d}".format(prefix, index),
                    "location": P5D3.P5D1A.RANGEFINDER_LOCATION,
                    "rotation": [0.0, 0.0, float(channel_yaw + float(local_yaw_deg))],
                    "configuration": {
...
def _scenario_config(
    *,
    ...
    return {
        "name": "phase5e_{0}".format(run_kind),
...
def _aggregate_fan_range(
    state_env: Dict[str, Any],
    agent_name: str,
    channel_id: str = DEFAULT_CAMERA_CHANNEL_ID,
) -> List[float]:
    values: List[float] = []
    for name in _fan_rangefinder_names(channel_id):
...
def _capture_agent(
    ...
    channel_captures: Dict[str, Dict[str, Any]] = {}
    for channel in CAMERA_CHANNELS:
        channel_id = str(channel["id"])
        rgb = get_sensor_vector(state_env, agent_name, str(channel["camera_name"]))
        channel_captures[channel_id] = {
            "camera_channel_id": channel_id,
            "camera_name": str(channel["camera_name"]),
            "camera_yaw_deg": float(channel["yaw_deg"]),
            "rgb": None if rgb is None else np.asarray(rgb).copy(),
            "fan_range": np.asarray(_aggregate_fan_range(state_env, agent_name, channel_id), dtype=float).copy(),
        }
    front = channel_captures[DEFAULT_CAMERA_CHANNEL_ID]
    return {
        "agent_name": agent_name,
        "rgb": front["rgb"],
        "fan_range": front["fan_range"],
        "camera_channels": channel_captures,
...
def _iter_rolling_rgb_captures(
    ...
    *,
    ...
    agent_name: str,
    camera_channel_id: str = DEFAULT_CAMERA_CHANNEL_ID,
    max_offset_ticks: int = RGB_SYNC_WINDOW_TICKS,
) -> List[Dict[str, Any]]:
    ...
    for step_key, step_dict in by_step.items():
        ...
        for tick_key, captures_by_agent in step_dict.items():
            ...
            if not isinstance(capture, dict):
                continue
            channel_capture = capture.get("camera_channels", {}).get(camera_channel_id)
            rgb = channel_capture.get("rgb") if isinstance(channel_capture, dict) else capture.get("rgb")
            if rgb is None:
                continue
            ...
            rows.append(
                {
                    ...
                    "offset": int(offset),
                    "capture": capture,
                    "channel_capture": channel_capture if isinstance(channel_capture, dict) else None,
                }
            )
    return rows
...
def _select_synced_rgb_pair(
    *,
    ...
    result: Dict[str, Any] = {
        ...
        "synced_target_channel_capture": None,
        "synced_baseline_channel_capture": None,
    }
    ...
    for target_row in target_candidates:
        ...
        target_channel_capture = target_row.get("channel_capture") or {}
        target_rgb = target_channel_capture.get("rgb", target_capture.get("rgb"))
        target_offset = int(target_row["offset"])
        for baseline_row in baseline_candidates:
            baseline_capture = baseline_row["capture"]
            baseline_channel_capture = baseline_row.get("channel_capture") or {}
            baseline_rgb = baseline_channel_capture.get("rgb", baseline_capture.get("rgb"))
            ...
            candidates.append(
                {
                    ...
                    "target_channel_capture": target_channel_capture,
                    "baseline_channel_capture": baseline_channel_capture,
                }
            )
    ...
    result.update(
        {
            ...
            "synced_target_channel_capture": best["target_channel_capture"],
            "synced_baseline_channel_capture": best["baseline_channel_capture"],
        }
    )
    return result
    ...
    *,
    ...
    def _act_and_capture(
        ...
        for usv_id in (0, 1):
            ...
            channel_summaries = {
                channel_id: P5D3.P5D1A._range_summary(channel_capture.get("fan_range"))
                for channel_id, channel_capture in capture.get("camera_channels", {}).items()
                if isinstance(channel_capture, dict)
            }
            range_summary = P5D3.P5D1A._range_summary(capture["fan_range"])
            any_channel_hit = any(bool(row.get("any_hit", False)) for row in channel_summaries.values())
            hit_channels = [key for key, row in channel_summaries.items() if bool(row.get("any_hit", False))]
            any_channel_rgb = any(
                channel_capture.get("rgb") is not None
                for channel_capture in capture.get("camera_channels", {}).values()
                if isinstance(channel_capture, dict)
            )
            ...
            tick_rows.append(
                {
                    ...
                    "any_camera_rgb_present": bool(any_channel_rgb),
                    "rangefinder_present": bool(range_summary.get("present", False)),
                    "any_rangefinder_hit": bool(range_summary.get("any_hit", False)),
                    "any_channel_rangefinder_hit": bool(any_channel_hit),
                    "rangefinder_hit_channels": hit_channels,
...
def _event_from_capture(
    *,
    ...
    channel = _camera_channel(camera_channel_id)
    channel_capture = capture.get("camera_channels", {}).get(camera_channel_id)
    if not isinstance(channel_capture, dict):
        channel_capture = {
            "rgb": capture.get("rgb"),
            "fan_range": capture.get("fan_range"),
            "camera_channel_id": DEFAULT_CAMERA_CHANNEL_ID,
            "camera_name": P5D3.RGB_CAMERA_NAME,
            "camera_yaw_deg": 0.0,
        }
    fan_range = channel_capture.get("fan_range")
    range_summary = P5D3.P5D1A._range_summary(fan_range)
    ...
    rgb = channel_capture.get("rgb")
    baseline_channel_capture = None
    if isinstance(baseline_capture, dict):
        baseline_channel_capture = baseline_capture.get("camera_channels", {}).get(camera_channel_id)
    baseline_rgb = None
    if isinstance(baseline_channel_capture, dict):
        baseline_rgb = baseline_channel_capture.get("rgb")
    elif baseline_capture is not None:
        baseline_rgb = baseline_capture.get("rgb")
    if rgb_sync.get("rgb_range_sync_applied") is True:
        synced_target_capture = rgb_sync.get("synced_target_channel_capture") or rgb_sync.get("synced_target_capture")
        synced_baseline_capture = rgb_sync.get("synced_baseline_channel_capture") or rgb_sync.get("synced_baseline_capture")
    ...
    rgb_sync_event = {
        ...
        if key
        not in (
            "synced_target_capture",
            "synced_baseline_capture",
            "synced_target_channel_capture",
            "synced_baseline_channel_capture",
        )
    }
    ...
    camera_yaw = float(channel.get("yaw_deg", 0.0))
    world_bearing = None if bearing is None else float(bearing) + camera_yaw
    ...
    estimated = _estimate_world_position(observer_location, observer_heading, world_bearing, matched_range)
    ...
    return {
        ...
        "agent_id": agent_name,
        "sensor_id": "{0}+{1}".format(str(channel["camera_name"]), _fan_rangefinder_prefix(camera_channel_id)),
        "camera_channel_id": camera_channel_id,
        "camera_sensor_name": str(channel["camera_name"]),
    ...
    *,
    ...
    if display_live and run_kind == "target":
        live_viewer = LiveSearchViewer(
            ...
            map_shape=nav_map_prior.shape,
            title="Phase 5E Live Dual-USV Search",
            fps=display_fps,
    ...
    adapter = ReusableMainlinePerceptionAdapter(adapter_id="phase5e_{0}".format(run_kind))
    adapter.reset_episode("phase5e_{0}".format(run_kind))
    launch_ok = False
    error = ""
    ...
    try:
        with holoocean.make(scenario_cfg=scenario_cfg) as env:
            ...
            for actor in moving_actor_names:
                env.act(actor, np.zeros(2, dtype=np.float32))
            state_env = env.tick()
            initial_observer_world = {}
            for usv_id in (0, 1):
                selected_initial = _select_sensor(state_env, _agent_name(usv_id), phys_state["last_known"][usv_id])
                initial_observer_world[usv_id] = list(selected_initial["curr_pos"])
                phys_state["last_known"][usv_id] = list(selected_initial["curr_pos"])
            _apply_bridge_clue_observations(
                state=state,
                adapter_config=adapter_config,
                policy_step=0,
                run_kind=run_kind,
                observer_world_by_usv=initial_observer_world,
                source_world_location=target_world,
                observation_phase="initial_holoocean_pose",
            )
            ...
            for step in range(1, int(config["max_policy_steps"]) + 1):
                        ...
                        for agent_name in OBSERVER_AGENT_NAMES:
                            ...
                            for channel in CAMERA_CHANNELS:
                                event = _event_from_capture(
                                    ...
                                    camera_channel_id=str(channel["id"]),
                ...
                bridge_clue_rows = _apply_bridge_clue_observations(
                    state=state,
                    adapter_config=adapter_config,
                    policy_step=step,
                    run_kind=run_kind,
                    observer_world_by_usv={
                        0: list(phys_state["last_known"][0]),
                        1: list(phys_state["last_known"][1]),
                    },
                    source_world_location=target_world,
                    observation_phase="post_step_holoocean_pose",
                )
                update_row.update(
                    {
                        "run_kind": run_kind,
                        ...
                        "baseline_target_induced_clue_used_for_gp": False,
                        "bridge_clue_observation_used_for_gp": True,
                        "bridge_clue_observation_count_this_step": len(bridge_clue_rows),
                        "clue_directly_updates_found_mask": False,
                    }
                )
    ...
    return {
        ...
        "clue_rows": list(state.get("bridge_clue_rows", [])),
        "bridge_clue_gp_update_count": int(state.get("bridge_clue_gp_update_count", 0)),
        "gp_observation_count_final": int(getattr(state.get("gp_field"), "n_obs", len(state.get("gp_field").y_obs) if state.get("gp_field") is not None else 0)),
...
def _skipped_live_case(run_kind: str, reason: str) -> Dict[str, Any]:
    return {
        "run_kind": run_kind,
        "launch_ok": True,
        "skipped": True,
        ...
        "physical_failure": False,
        "initial_target_cell": None,
        "target_world_location": None,
        "found_mask_final": [],
        "find_times_final": [],
        "all_found": False,
        "all_found_step": None,
        "completed_steps": 0,
        "raw_captures_by_step_tick": {},
        "tick_trace": [],
        "policy_trace": [],
        "event_rows": [],
        "fusion_rows": [],
...
def _channel_capture(capture: Optional[Dict[str, Any]], channel_id: str) -> Optional[Dict[str, Any]]:
    if not isinstance(capture, dict):
        return None
    channel_capture = capture.get("camera_channels", {}).get(channel_id)
    if isinstance(channel_capture, dict):
        return channel_capture
    return {
        "rgb": capture.get("rgb"),
        "fan_range": capture.get("fan_range"),
        "camera_channel_id": DEFAULT_CAMERA_CHANNEL_ID,
...
def _raw_frame_case_row(
    *,
    ...
    camera_channel_id = str(event.get("camera_channel_id") or DEFAULT_CAMERA_CHANNEL_ID)
    ...
    target_channel = _channel_capture(target_capture, camera_channel_id)
    baseline_channel = _channel_capture(baseline_capture, camera_channel_id)
    exact_baseline_channel = _channel_capture(exact_baseline_capture, camera_channel_id)
    synced_target_channel = _channel_capture(synced_target_capture, camera_channel_id)
    synced_baseline_channel = _channel_capture(synced_baseline_capture, camera_channel_id)
    target_rgb = (
        synced_target_channel.get("rgb")
        if isinstance(synced_target_channel, dict) and synced_target_channel.get("rgb") is not None
        else (None if target_channel is None else target_channel.get("rgb"))
    )
    baseline_rgb = (
        synced_baseline_channel.get("rgb")
        if isinstance(synced_baseline_channel, dict) and synced_baseline_channel.get("rgb") is not None
        else (None if baseline_channel is None else baseline_channel.get("rgb"))
    )
    fan_range = None if target_channel is None else target_channel.get("fan_range")
    range_summary = P5D3.P5D1A._range_summary(fan_range)
    rgb_diff = P5D3.P5D1A._rgb_diff_signature(target_rgb, baseline_rgb)
    sphere_blob = P5D3.P5D1A._sphere_blob_signature(target_rgb, baseline_rgb)
    ...
    base = os.path.join(
        PHASE_DIR,
        "visuals",
        "phase5e_step{0:02d}_tick{1:03d}_{2}".format(step, tick, agent_name),
    )
    if save_visuals:
        visuals = {
    ...
    if exact_baseline_capture is not None:
        exact_rgb_diff = P5D3.P5D1A._rgb_diff_signature(
            target_rgb,
            None if exact_baseline_channel is None else exact_baseline_channel.get("rgb"),
        )
    return {
        "run_kind": "target",
        "policy_step": step,
        ...
        "agent_id": agent_name,
        "camera_channel_id": camera_channel_id,
        "camera_sensor_name": event.get("camera_sensor_name"),
        ...
        "exact_target_rgb_present": None if target_channel is None else target_channel.get("rgb") is not None,
        "exact_baseline_rgb_present": None
        if exact_baseline_channel is None
        else exact_baseline_channel.get("rgb") is not None,
        ...
        "synced_target_rgb_present": None
        if synced_target_channel is None
        else synced_target_channel.get("rgb") is not None,
        "synced_baseline_rgb_present": None
        if synced_baseline_channel is None
        else synced_baseline_channel.get("rgb") is not None,
...
def _build_raw_frame_sync_diagnostic(
    *,
    ...
    return {
        "phase_name": PHASE_NAME,
        "diagnostic_name": "phase5e_raw_frame_baseline_sync_diagnostic",
...
def _run_compile_log(path: str) -> None:
    ...
    result = subprocess.run(
        [
            ...
            "-m",
            "py_compile",
            os.path.join(PHASE_DIR, "scripts", "bridge_clue_sensor_search_probe.py"),
        ],
        cwd=os.path.abspath("."),
        check=False,
        text=True,
...
def _build_summary(
    *,
    ...
    target_clue_rows = [row for row in clue_rows if row.get("run_kind") == "target"]
    clue_rows_used_for_gp = [row for row in clue_rows if row.get("used_for_gp_observation") is True]
    clue_rows_used_for_detection = [row for row in clue_rows if row.get("used_for_detection") is True]
    ...
    return {
        ...
        "phase_goal": "holoocean_bridge_local_clue_sensor_drives_baseline_gp_while_rgb_rf_remains_only_found_mask_trigger",
        "phase_goal_completed": phase_goal_completed,
        "phase_stage_completion": {
            "5E-0": {
                "status": "completed",
                "evidence": "independent_phase_dir_and_protected_core_status",
            },
            "5E-1": {
                "status": "completed" if bool(config.get("bridge_clue_sensor_enabled")) else "failed",
                "evidence": "bridge_clue_sensor_config_and_trace",
            },
            "5E-2": {
                "status": "completed"
                if clue_rows_used_for_gp and config.get("baseline_target_induced_clue_used_for_gp") is False
                else "failed",
                "evidence": "bridge_clue_observation_trace_and_summary_flags",
            },
            "5E-3": {
                "status": "completed"
                if clue_rows_used_for_detection == [] and mainline_found_mask_updated
                else "failed",
                "evidence": "clue_trace_used_for_detection_false_and_rgb_rf_update_trace",
            },
            "5E-4": {
                "status": "pending_until_audit_script_runs",
                "evidence": "phase5e_audit_json_and_stage_report",
            },
        },
        ...
        "live_capture_primary_detection_source": True,
        "bridge_clue_sensor_enabled": bool(config.get("bridge_clue_sensor_enabled")),
        "bridge_clue_sensor_id": config.get("bridge_clue_sensor_id"),
        "bridge_clue_sensor_role": config.get("bridge_clue_sensor_role"),
        "bridge_clue_sensor_footprint": config.get("bridge_clue_sensor_footprint"),
        "bridge_clue_source_model": config.get("bridge_clue_source_model"),
        "bridge_clue_observation_used_for_gp": bool(config.get("bridge_clue_observation_used_for_gp")),
        "bridge_clue_observation_count": len(clue_rows),
        "target_bridge_clue_observation_count": len(target_clue_rows),
        "bridge_clue_rows_used_for_gp_count": len(clue_rows_used_for_gp),
        "bridge_clue_rows_used_for_detection_count": len(clue_rows_used_for_detection),
        "bridge_clue_registered_source_not_exposed_to_planner": all(
            row.get("registered_source_not_exposed_to_planner") is True for row in clue_rows
        ),
        "baseline_target_induced_clue_used_for_gp": bool(config.get("baseline_target_induced_clue_used_for_gp")),
        "baseline_target_induced_clue_sampling_disabled_in_phase5e": bool(
            config.get("baseline_target_induced_clue_sampling_disabled_in_phase5e")
        ),
        "gp_clue_source": config.get("gp_clue_source"),
        "clue_directly_updates_found_mask": any(
            row.get("directly_updates_found_mask") is True
            or row.get("detected_mask_from_clue") is True
            or row.get("found_mask_from_clue") is True
            for row in clue_rows
        ),
        "rgb_rf_detection_updates_found_mask": bool(
            mainline_found_mask_updated
            and any(int(row.get("source_accepted_candidate_count", 0) or 0) > 0 for row in target_updates)
        ),
        "rgb_range_sync_fix_enabled": True,
        "rgb_range_sync_strategy": RGB_SYNC_STRATEGY,
        "rgb_sync_window_ticks": RGB_SYNC_WINDOW_TICKS,
...
def _mark_target_only_demo_summary(
    summary: Dict[str, Any],
    *,
    calibration_run: Dict[str, Any],
    target_run: Dict[str, Any],
) -> Dict[str, Any]:
    demo_goal_completed = bool(
        target_run.get("launch_ok") is True
        and target_run.get("all_found") is True
        and summary.get("target_live_detection_success") is True
    ...
    summary.update(
        {
            "target_only_demo": True,
            "target_only_demo_goal_completed": demo_goal_completed,
            "audit_not_full": True,
            "formal_audit_evidence": False,
            "phase_completed": False,
            "phase_goal_completed": False,
            "phase_status": (
                "target_only_demo_all_found_no_stop_scan_closed"
                if demo_goal_completed
                else "target_only_demo_live_detection_not_closed"
            ),
            "run_cases": ["calibration", "target"],
            "skipped_run_cases": ["teammate_only"],
            "teammate_negative_case_executed": False,
            "teammate_negative_case_required_for_formal_audit": True,
            "all_runs_launch_ok": bool(calibration_run.get("launch_ok") and target_run.get("launch_ok")),
            "teammate_only_no_false_positive": None,
            "teammate_false_positive_steps": None,
            "teammate_only_false_positive_count": None,
            "teammate_events_accepted_count": None,
            "teammate_accepted_candidate_from_rgb_sync_count": None,
            "demo_note": (
                "Target-only demo skips the teammate-only negative run to reduce HoloOcean launches; "
                "use the full default run plus phase5e audit for formal evidence."
            ),
        }
    )
    return summary
...
def _build_summary_md(summary: Dict[str, Any]) -> str:
    title = (
        ...
        if summary.get("target_only_demo") is True
        else "# Phase 5E Bridge Clue Sensor Search Summary"
    )
    lines = [
        title,
        "",
        "- Target-Only Demo: `{0}`".format(summary.get("target_only_demo", False)),
        "- Audit Not Full: `{0}`".format(summary.get("audit_not_full", False)),
        "- Target-Only Demo Goal Completed: `{0}`".format(
            summary.get("target_only_demo_goal_completed")
        ),
        ...
        "- Bridge Clue Sensor Enabled: `{0}`".format(summary.get("bridge_clue_sensor_enabled")),
        "- Bridge Clue Observation Count: `{0}`".format(summary.get("bridge_clue_observation_count")),
        "- Bridge Clue Rows Used For GP: `{0}`".format(summary.get("bridge_clue_rows_used_for_gp_count")),
        "- Baseline Target-Induced Clue Used For GP: `{0}`".format(
            summary.get("baseline_target_induced_clue_used_for_gp")
        ),
        "- Clue Directly Updates Found Mask: `{0}`".format(summary.get("clue_directly_updates_found_mask")),
        "- RGB/RF Detection Updates Found Mask: `{0}`".format(summary.get("rgb_rf_detection_updates_found_mask")),
        ...
        "- Phase Stage Completion: `{0}`".format(summary.get("phase_stage_completion")),
        "",
        (
            ...
            else "Phase 5E exercises the real planner callback, bridge-local clue observations, and continuous HoloOcean capture path. If it does not close all_found, inspect clue trace and RGB/RF evidence."
        ),
        "",
    ]
    return "\n".join(lines)
...
def run_live_planner_callback_search(
    *,
    ...
    paths = _output_paths(target_only_demo=target_only_demo)
    started = time.perf_counter()
    ...
    print("Running 5E live calibration/reference planner callback run")
    ...
    print("Running 5E live target planner callback run")
    ...
    if target_only_demo:
        print("Skipping 5E teammate-only run because --target-only-demo is enabled")
        teammate_run = _skipped_live_case(
            "teammate_only",
            "--target-only-demo skips the negative teammate-only audit segment",
        )
    else:
        print("Running 5E live teammate-only planner callback run")
    ...
    all_clue_rows = (
        list(calibration_run.get("clue_rows", []))
        + list(target_run.get("clue_rows", []))
        + list(teammate_run.get("clue_rows", []))
    )
    all_policy = (
    ...
    summary = _build_summary(
        ...
        clue_rows=all_clue_rows,
        policy_rows=all_policy,
        tick_rows=all_ticks,
        started=started,
    )
    if target_only_demo:
        summary = _mark_target_only_demo_summary(
            summary,
            calibration_run=calibration_run,
            target_run=target_run,
        )
    ...
    git_doc = {
        "phase_name": PHASE_NAME,
        "target_only_demo": bool(target_only_demo),
        "audit_not_full": bool(target_only_demo),
    ...
    _save_json(paths["config_json"], config)
    _save_json(paths["production_contract_json"], mainline_perception_adapter_contract())
    _save_json(paths["calibration_run_json"], _strip_run_raw(calibration_run))
    _save_json(paths["target_run_json"], _strip_run_raw(target_run))
    _save_json(paths["teammate_run_json"], _strip_run_raw(teammate_run))
    _save_json(paths["tick_trace_json"], all_ticks)
    _save_csv(paths["tick_trace_csv"], all_ticks)
    _save_json(paths["policy_trace_json"], all_policy)
    _save_csv(paths["policy_trace_csv"], all_policy)
    _save_json(paths["events_json"], all_events)
    _save_csv(paths["events_csv"], all_events)
    _save_json(paths["fusion_json"], all_fusion)
    _save_csv(paths["fusion_csv"], all_fusion)
    _save_json(paths["adapter_trace_json"], all_adapter)
    _save_csv(paths["adapter_trace_csv"], all_adapter)
    _save_json(paths["mainline_update_trace_json"], all_updates)
    _save_csv(paths["mainline_update_trace_csv"], all_updates)
    _save_json(paths["clue_trace_json"], all_clue_rows)
    _save_csv(paths["clue_trace_csv"], all_clue_rows)
    _save_json(paths["raw_frame_sync_json"], raw_frame_sync)
    _save_csv(paths["raw_frame_sync_csv"], list(raw_frame_sync.get("rows", [])))
    _save_json(paths["summary_json"], summary)
    _save_json(paths["git_status_json"], git_doc)
    ...
    print("Summary written to: {0}".format(paths["summary_md"]))
    return summary
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Run only calibration/reference plus target live search, skipping the teammate-only "
            "negative audit segment. Writes separate target-only demo artifacts."
        ),
    )
    ...
    if args.mode == "live":
        summary = run_live_planner_callback_search(
            ...
            target_only_demo=args.target_only_demo,
        )
