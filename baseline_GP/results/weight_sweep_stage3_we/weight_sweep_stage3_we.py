...
# 原始经验权重，仅用于标记和对照。
ORIGINAL_WEIGHTS = (6.5, 3.0, -0.35)

# 第二阶段确认的 ws/wr 候选。
STAGE2_SELECTED_WS = 6.5
STAGE2_SELECTED_WR = 2.0
STAGE2_SELECTED_WE = -0.35
STAGE2_SELECTED_WEIGHTS = (
    STAGE2_SELECTED_WS,
    STAGE2_SELECTED_WR,
    STAGE2_SELECTED_WE,
)
...
OUTPUT_DIR = (
    Path("baseline_GP")
    / "results"
    / "weight_sweep_stage3_we"
    / f"stage3_we_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
)


def build_stage3_weight_configs():
    """Return stage-3 configs that sweep w_e with ws/wr fixed.

    Stage 3:
      w_s fixed at 6.5
      w_r fixed at 2.0
      w_e in {-0.15, -0.25, -0.35, -0.45, -0.55}
    """
    we_values = (-0.15, -0.25, -0.35, -0.45, -0.55)

    configs = []
    for we in we_values:
        configs.append(
            {
                "stage": "stage3_we_axis",
                "w_s": float(STAGE2_SELECTED_WS),
                "w_r": float(STAGE2_SELECTED_WR),
                "w_e": float(we),
            }
        )
    return configs
...
def _same_weight(
    ...
    return (
        abs(float(row["w_s"]) - float(weight[0])) < 1e-9
        and abs(float(row["w_r"]) - float(weight[1])) < 1e-9
        and abs(float(row["w_e"]) - float(weight[2])) < 1e-9
    )
...
def summarize_by_config(rows: list[dict]) -> list[dict]:
    ...
    for idx, row in enumerate(summary_rows, start=1):
        ...
        row["is_original_weight"] = _same_weight(row, ORIGINAL_WEIGHTS)
        row["is_stage2_selected_weight"] = _same_weight(row, STAGE2_SELECTED_WEIGHTS)
...
def main() -> None:
    ...
    weight_configs = build_stage3_weight_configs()
    ...
    config_snapshot = {
        ...
        "clue_sigma_m": 15.0,
        "fixed_weights": {
            "w_s": STAGE2_SELECTED_WS,
            "w_r": STAGE2_SELECTED_WR,
        },
        "swept_weight": "w_e",
        "original_weights": {
            "w_s": ORIGINAL_WEIGHTS[0],
            "w_r": ORIGINAL_WEIGHTS[1],
            "w_e": ORIGINAL_WEIGHTS[2],
        },
        "stage2_selected_weights": {
            "w_s": STAGE2_SELECTED_WEIGHTS[0],
            "w_r": STAGE2_SELECTED_WEIGHTS[1],
            "w_e": STAGE2_SELECTED_WEIGHTS[2],
        },
    ...
    stage2_selected_config = next(
        (row for row in summary_rows if row.get("is_stage2_selected_weight")),
        None,
    )
    original_config = next(
        (row for row in summary_rows if row.get("is_original_weight")),
        None,
    )

    with (OUTPUT_DIR / "stage3_summary.json").open("w", encoding="utf-8") as f:
        json.dump(
            {
                "output_dir": str(OUTPUT_DIR),
                "best_config": summary_rows[0] if summary_rows else None,
                "stage2_selected_config": stage2_selected_config,
