# -*- coding: utf-8 -*-
"""Run anomaly_tail_quantile sweep without changing formal runners."""
...
def _run_episode(
    *,
    ...
    common = {
        ...
        "anomaly_tail_quantile": float(quantile),
    ...
    result["anomaly_tail_quantile"] = float(quantile)
...
def run_group(*, system: str, motion_mode: str, output_root: Path, resume: bool) -> Path:
    ...
    config = {
        ...
        "max_iters": MAX_ITERS,
        "clue_acquisition_mode": CLUE_MODE,
        "anomaly_tail_quantiles": list(QUANTILES),
        "anomaly_weight_lambda": ANOMALY_WEIGHT_LAMBDA,
    ...
    results_by_q_map: dict[float, dict[str, list[dict[str, Any]]]] = {
        q: {map_kind: [] for map_kind in MAP_KINDS} for q in QUANTILES
    }
    for q in QUANTILES:
        for map_kind in MAP_KINDS:
            result_path = raw_dir / _q_label(q) / map_kind / "episode_results.json"
            existing = _load_results(result_path) if resume else []
            results_by_q_map[q][map_kind] = existing
            completed = {int(item["episode_seed"]) for item in existing if item.get("episode_seed") is not None}
            for seed in EPISODE_SEEDS:
                if seed in completed:
                    continue
                print(
                    "anomaly quantile sweep | "
                    f"system={system} | motion={motion_mode} | q={q:.2f} | "
                ...
                results_by_q_map[q][map_kind].append(result)
                _save_results(result_path, results_by_q_map[q][map_kind])

    episode_rows: list[dict[str, Any]] = []
    for q in QUANTILES:
        for map_kind in MAP_KINDS:
            episode_rows.extend(results_by_q_map[q][map_kind])
    by_map_rows, overall_rows = _summary_rows(
        system=system,
        motion_mode=motion_mode,
        results_by_q_map=results_by_q_map,
    )
    _write_csv(output_dir / "episode_results.csv", episode_rows)
    _write_csv(output_dir / "summary_by_quantile_and_map.csv", by_map_rows)
    _write_csv(output_dir / "summary_by_quantile.csv", overall_rows)
    return output_dir
...
def main() -> None:
    ...
    print(f"quantile_sweep_output_dir={out}", flush=True)
