...
OUTPUT_DIR = (
    Path("baseline_GP")
    / "results"
    / "kappa_ablation_single_usv_anomaly"
    / f"kappa_ablation_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
)
    ...
    *,
    ...
    return {
        ...
        "path_invalidations_per_episode": int(
            result.get("path_invalidations_per_episode", 0)
        ),
        "clearance_penalty_raw_mean": float(
            result.get("clearance_penalty_raw_mean", 0.0)
        ),
        "near_obstacle_step_ratio": float(result.get("near_obstacle_step_ratio", 0.0)),
        "min_clearance_cells_mean": float(result.get("min_clearance_cells_mean", 0.0)),
    }
...
def _delta_rows(summary_rows: list[dict]) -> list[dict]:
    ...
    for motion_mode in TARGET_MOTION_MODES:
        for map_kind in MAP_KINDS:
            ...
            row["success_all_found_rate_delta"] = (
                float(no_kappa["success_all_found_rate"] or 0.0)
...
def main() -> None:
    ...
    config_snapshot = {
        "experiment": "kappa_ablation_single_usv_anomaly",
        ...
        for motion_mode in TARGET_MOTION_MODES:
            for map_kind in MAP_KINDS:
                for seed in EPISODE_SEEDS:
                    ...
                    print(
                        f"[{done}/{total}] "
                        f"planner={planner_mode} motion={motion_mode} "
